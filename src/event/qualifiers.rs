...
async fn qualifiers_form(mut transaction: Transaction<'_, Postgres>, me: User, uri: Origin<'_>, csrf: Option<&CsrfToken>, event: Data<'_>, is_started: bool, ctx: Context<'_>) -> Result<RawHtml<String>, event::Error> {
    ...
    let seeding_race = sqlx::query_as!(RaceRow,
        "SELECT id AS \"id: Id<Races>\", round, start, room FROM races WHERE series = $1 AND event = $2 AND phase = 'Seeding' LIMIT 1",
        event.series as _, &event.event
    )
...
#[rocket::post("/event/<series>/<event>/qualifiers/create-seeding-race", data = "<form>")]
pub(crate) async fn post_seeding_race(pool: &State<PgPool>, discord_ctx: &State<RwFuture<DiscordCtx>>, http_client: &State<reqwest::Client>, me: User, uri: Origin<'_>, csrf: Option<CsrfToken>, series: Series, event: &str, form: Form<Contextual<'_, SeedingRaceForm>>) -> Result<RedirectOrContent, StatusOrError<event::Error>> {
    let mut transaction = pool.begin().await?;
    let event_data = Data::new(&mut transaction, series, event).await?.ok_or(StatusOrError::Status(Status::NotFound))?;
    let mut form = form.into_inner();
    form.verify(&csrf);
    ...
    Ok(if let Some(ref value) = form.value {
        if form.context.errors().next().is_some() {
            ...
            match crate::discord_scheduled_events::create_discord_scheduled_event(&*discord_ctx.read().await, &mut transaction, &mut race, &event_data, http_client.inner()).await {
                Ok(()) => { race.save(&mut transaction).await?; }
                Err(e) => { eprintln!("Failed to create Discord scheduled event for seeding race {}: {}", race.id, e); }
            }
            transaction.commit().await?;
            RedirectOrContent::Redirect(Redirect::to(uri!(get(series, event))))
        }
...
#[rocket::post("/event/<series>/<event>/qualifiers/seeding-race/<race_id>/delete", data = "<form>")]
pub(crate) async fn delete_seeding_race(pool: &State<PgPool>, me: User, csrf: Option<CsrfToken>, series: Series, event: &str, race_id: Id<Races>, form: Form<Contextual<'_, DeleteForm>>) -> Result<Redirect, StatusOrError<event::Error>> {
    let mut transaction = pool.begin().await?;
    let event_data = Data::new(&mut transaction, series, event).await?.ok_or(StatusOrError::Status(Status::NotFound))?;
    let mut form = form.into_inner();
    form.verify(&csrf);
...
async fn edit_seeding_race_form(mut transaction: Transaction<'_, Postgres>, me: User, uri: Origin<'_>, csrf: Option<&CsrfToken>, event: Data<'_>, race_id: Id<Races>, ctx: Context<'_>) -> Result<RawHtml<String>, event::Error> {
    let header = event.header(&mut transaction, Some(&me), Tab::Qualifiers, false).await?;
...
pub(crate) async fn get_edit_seeding_race(pool: &State<PgPool>, me: User, uri: Origin<'_>, csrf: Option<CsrfToken>, series: Series, event: String, race_id: Id<Races>) -> Result<RawHtml<String>, StatusOrError<event::Error>> {
    let mut transaction = pool.begin().await?;
    let event_data = Data::new(&mut transaction, series, &event).await?.ok_or(StatusOrError::Status(Status::NotFound))?;
    if !me.is_global_admin() && !event_data.organizers(&mut transaction).await?.contains(&me) {
        return Err(StatusOrError::Status(Status::Forbidden));
    ...
    Ok(edit_seeding_race_form(transaction, me, uri, csrf.as_ref(), event_data, race_id, Context::default()).await?)
}

#[rocket::post("/event/<series>/<event>/qualifiers/seeding-race/<race_id>/edit", data = "<form>")]
pub(crate) async fn post_edit_seeding_race(pool: &State<PgPool>, discord_ctx: &State<RwFuture<DiscordCtx>>, http_client: &State<reqwest::Client>, me: User, uri: Origin<'_>, csrf: Option<CsrfToken>, series: Series, event: &str, race_id: Id<Races>, form: Form<Contextual<'_, SeedingRaceForm>>) -> Result<RedirectOrContent, StatusOrError<event::Error>> {
    let mut transaction = pool.begin().await?;
    let event_data = Data::new(&mut transaction, series, event).await?.ok_or(StatusOrError::Status(Status::NotFound))?;
    let mut form = form.into_inner();
    form.verify(&csrf);
    ...
    Ok(if let Some(ref value) = form.value {
        if form.context.errors().next().is_some() {
            RedirectOrContent::Content(edit_seeding_race_form(transaction, me, uri, csrf.as_ref(), event_data, race_id, form.context).await?)
        } else {
            let start = match NaiveDateTime::parse_from_str(&value.race_start, "%Y-%m-%dT%H:%M") {
                Ok(naive_dt) => DateTime::<Utc>::from_naive_utc_and_offset(naive_dt, Utc),
                Err(_) => {
                    form.context.push_error(form::Error::validation("Invalid start time format").with_name("race_start"));
                    return Ok(RedirectOrContent::Content(edit_seeding_race_form(transaction, me, uri, csrf.as_ref(), event_data, race_id, form.context).await?));
                }
            };
...

            {
                ...
                match Race::from_id(&mut transaction, http_client.inner(), race_id).await {
                    Ok(race) => {
                        if let Err(e) = crate::discord_scheduled_events::update_discord_scheduled_event(&*discord_ctx.read().await, &mut transaction, &race, &event_data, http_client.inner()).await {
                            eprintln!("Failed to update Discord scheduled event for seeding race {}: {}", race_id, e);
                        }
                        let _ = transaction.commit().await;
                    }
                    Err(e) => eprintln!("Failed to load seeding race {} for Discord event update: {}", race_id, e),
                }
            }
