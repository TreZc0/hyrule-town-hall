...
pub(super) fn label(id: &str, field: &str, title: &str) -> RawHtml<String> {
    let paragraphs: &[&str] = match field {
        ...
        "seed_config" => &[
            ...
            "While requests are paused and settings are not locked, you can correct an unused pool’s baseline. Saving replaces its private seed data and queues the existing slots for generation again. Any in-progress generation using the old baseline is discarded. This is blocked after a seed is assigned or released, or a live room opens or its seed generation starts. Generator and enabled-state changes remain protected once seed material exists. Display name and order can still be edited.",
        ],
        "enabled" => &[
