...
pub(super) fn seed_choice_help() -> RawHtml<String> {
    html! {
        ...
        details {
            ...
            p : "Choose when random player choices resolve in event setup: On race creation / import, On room opening, or On seed reveal (the default). The Seed Config JSON field is choice_resolution, with values race_creation, room_opening and seed_rolling. Creation waits until all participants are registered. Scheduling threads show agreed settings and rules after the welcome message and before the scheduling instructions. Random choices remain labeled as random unless creation/import is selected, which also reveals their saved results there. Room opening posts in each live room or private async thread, and falls back to seed reveal when no room opens. Seed reveal posts settings beside the seed for each participant; advance seed generation never announces them. The seed_rolling key is retained for compatibility. Practice rolls remain independent; pooled qualifiers use only baseline settings.";
