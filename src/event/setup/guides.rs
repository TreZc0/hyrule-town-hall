    ...
    html! {
        details(class = "configuration-guide") {
            ...
            details {
                ...
                p : "Ungrouped requirements appear first. Top-level sections follow sections-array order; within a section its own requirements appear in requirements-array order, then its child sections. Empty sections are hidden. The visual editor lets you add and remove sections, edit headings and parents, assign requirements, and swap order numbers. Save sections and order applies these changes together; ties keep their previous order. Arrows immediately change requirement array order without changing section assignments or saving pending field edits. Editing a requirement also lets you change its section and keeps the current assignment on save. The same grouping is used where player choices are shown for editing.";
        ...
        details(class = "configuration-guide") {
            ...
            details {
                ...
                : fields(&[
                    ...
                    ("choice_resolution", "optional string; default seed_rolling", "race_creation, room_opening or seed_rolling. Controls when saved choices are revealed: in the scheduling thread at creation/import, in each race room at opening, or beside the seed at reveal (seed_rolling). The Resolve random player choices selector in this form writes this property and takes precedence over the value you paste here."),
                ]);
            ...
            details {
                ...
                : fields(&[
                    ...
                    ("room_opening", "resolve when opening a room", "Posts settings in each race room or private async thread as it opens. Workflows without a room fall back to seed reveal. Subsequent seed generation uses that same saved decision."),
                    ("seed_rolling", "reveal beside the seed; default", "Choices are saved during generation but only posted beside the seed when it is revealed, separately in each private async thread. They are never posted to the scheduling thread. Rerolls reuse the saved choices. The seed_rolling key is retained for compatibility. Each game has its own saved result."),
                ]);
