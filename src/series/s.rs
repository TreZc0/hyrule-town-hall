...
impl Setting {
    pub(crate) fn description(&self) -> String {
        let options = iter::once(format!("default ({})", self.default_display)).chain(
            self.other
                .iter()
                .map(|(name, display, _)| format!("{name} ({display})")),
        );
        format!(
            "{}: {}",
            self.name,
            English
                .join_str_opt_with("or", options)
                .expect("has at least the default option")
        )
    }
}
...
pub(crate) const S7_SETTINGS: [Setting; 31] = [
    Setting {
        ...
        default_display: "6 med bridge, GCBK removed",
        other: &[(
            "open",
            "Open bridge, 6 med GCBK",
            || collect![format!("bridge") => json!("open"), format!("shuffle_ganon_bosskey") => json!("medallions")],
        )],
    },
    Setting {
        major: true,
        name: "deku",
        display: "Kokiri Forest",
        default_display: "Closed Deku",
        other: &[(
            "open",
            "Open Forest",
            || collect![format!("open_forest") => json!("open")],
        )],
    },
    Setting {
        major: true,
        name: "interiors",
        display: "Indoor ER",
        default_display: "Indoor ER Off",
        other: &[(
            "on",
            "Indoor ER On (All)",
            || collect![format!("shuffle_interior_entrances") => json!("all")],
        )],
    },
    Setting {
        major: true,
        name: "dungeons",
        display: "Dungeon ER",
        default_display: "Dungeon ER Off",
        other: &[(
            "on",
            "Dungeon ER On (no Ganon's Castle)",
            || collect![format!("shuffle_dungeon_entrances") => json!("simple")],
        )],
    },
    Setting {
        major: true,
        name: "grottos",
        display: "Grotto ER",
        default_display: "Grotto ER Off",
        other: &[(
            "on",
            "Grotto ER On",
            || collect![format!("shuffle_grotto_entrances") => json!(true)],
        )],
    },
    Setting {
        major: true,
        name: "shops",
        display: "Shopsanity",
        default_display: "Shopsanity Off",
        other: &[(
            "on",
            "Shopsanity 4",
            || collect![format!("shopsanity") => json!("4")],
        )],
    },
    Setting {
        major: true,
        name: "ow_tokens",
        display: "Overworld Tokens",
        default_display: "Overworld Tokens Off",
        other: &[(
            "on",
            "Overworld Tokens On",
            || collect![format!("tokensanity") => json!("overworld")],
        )],
    },
    Setting {
        major: true,
        ...
        default_display: "Dungeon Tokens Off",
        other: &[(
            "on",
            "Dungeon Tokens On",
            || collect![format!("tokensanity") => json!("dungeons")],
        )],
    },
    Setting {
        major: true,
        ...
        default_display: "Scrub Shuffle Off",
        other: &[(
            "on",
            "Scrub Shuffle On (Affordable)",
            || collect![format!("shuffle_scrubs") => json!("low")],
        )],
    },
    Setting {
        major: true,
        name: "keys",
        display: "Keys",
        default_display: "Own Dungeon Keys",
        other: &[
            (
                "keysy",
                "Keysy (both small and BK)",
                || collect![format!("shuffle_smallkeys") => json!("remove"), format!("shuffle_bosskeys") => json!("remove")],
            ),
            (
                "anywhere",
                "Keyrings anywhere (includes BK)",
                || collect![format!("shuffle_smallkeys") => json!("keysanity"), format!("key_rings_choice") => json!("all"), format!("keyring_give_bk") => json!(true)],
            ),
        ],
    },
    Setting {
        major: true,
        name: "required_only",
        display: "Guarantee Reachable Locations",
        default_display: "All Locations Reachable",
        other: &[(
            "on",
            "Required Only (Beatable Only)",
            || collect![format!("reachable_locations") => json!("beatable")],
        )],
    },
    Setting {
        major: true,
        ...
        default_display: "Zora's Fountain Closed",
        other: &[(
            "open",
            "Zora's Fountain Open (both ages)",
            || collect![format!("zora_fountain") => json!("open")],
        )],
    },
    Setting {
        major: true,
        ...
        default_display: "Shuffle Cows Off",
        other: &[(
            "on",
            "Shuffle Cows On",
            || collect![format!("shuffle_cows") => json!(true)],
        )],
    },
    Setting {
        major: true,
        ...
        default_display: "Shuffle Gerudo Card Off",
        other: &[(
            "on",
            "Shuffle Gerudo Card On",
            || collect![format!("shuffle_gerudo_card") => json!(true)],
        )],
    },
    Setting {
        major: true,
        ...
        other: &[("on", "3 Trials", || collect![format!("trials") => json!(3)])],
    },
    Setting {
        major: true,
        name: "door_of_time",
        display: "Open Door of Time",
        default_display: "Open Door of Time",
        other: &[(
            "closed",
            "Closed Door of Time",
            || collect![format!("open_door_of_time") => json!(false)],
        )],
    },
    Setting {
        major: false,
        ...
        default_display: "Random Starting Age",
        other: &[
            (
                "child",
                "Child Start",
                || collect![format!("starting_age") => json!("child")],
            ),
            (
                "adult",
                "Adult Start",
                || collect![format!("starting_age") => json!("adult")],
            ),
        ],
    },
    Setting {
        major: false,
        name: "random_spawns",
        display: "Random Spawns",
        default_display: "Random Spawns Off",
        other: &[(
            "on",
            "Random Spawns On (both ages)",
            || collect![format!("spawn_positions") => json!(["child", "adult"])],
        )],
    },
    Setting {
        major: false,
        ...
        default_display: "Start With Consumables On",
        other: &[(
            "none",
            "Start With Consumables Off",
            || collect![format!("start_with_consumables") => json!(false)],
        )],
    },
    Setting {
        major: false,
        name: "rupees",
        display: "Start With Max Rupees",
        default_display: "Start With Max Rupees Off",
        other: &[(
            "startwith",
            "Start With Max Rupees On",
            || collect![format!("start_with_rupees") => json!(true)],
        )],
    },
    Setting {
        major: false,
        name: "cuccos",
        display: "Anju's Chickens",
        default_display: "7 Chickens",
        other: &[(
            "1",
            "1 Chicken",
            || collect![format!("chicken_count") => json!(1)],
        )],
    },
    Setting {
        major: false,
        ...
        default_display: "Free Scarecrow Off",
        other: &[(
            "on",
            "Free Scarecrow On",
            || collect![format!("free_scarecrow") => json!(true)],
        )],
    },
    Setting {
        major: false,
        name: "camc",
        display: "CAMC",
        default_display: "CAMC: Size + Texture",
        other: &[(
            "off",
            "CAMC Off",
            || collect![format!("correct_chest_appearances") => json!("off")],
        )],
    },
    Setting {
        major: false,
        ...
        default_display: "Complete Mask Quest Off",
        other: &[(
            "complete",
            "Complete Mask Quest On",
            || collect![format!("complete_mask_quest") => json!(true), format!("fast_bunny_hood") => json!(false)],
        )],
    },
    Setting {
        major: false,
        ...
        default_display: "Blue Fire Arrows Off",
        other: &[(
            "on",
            "Blue Fire Arrows On",
            || collect![format!("blue_fire_arrows") => json!(true)],
        )],
    },
    Setting {
        major: false,
        name: "owl_warps",
        display: "Random Owl Warps",
        default_display: "Random Owl Warps Off",
        other: &[(
            "random",
            "Random Owl Warps On",
            || collect![format!("owl_drops") => json!(true)],
        )],
    },
    Setting {
        major: false,
        name: "song_warps",
        display: "Random Warp Song Destinations",
        default_display: "Random Warp Song Destinations Off",
        other: &[(
            "random",
            "Random Warp Song Destinations On",
            || collect![format!("warp_songs") => json!(true)],
        )],
    },
    Setting {
        major: false,
        name: "shuffle_beans",
        display: "Shuffle Magic Beans",
        default_display: "Shuffle Magic Beans Off",
        other: &[(
            "on",
            "Shuffle Magic Beans On",
            || collect![format!("shuffle_beans") => json!(true)],
        )],
    },
    Setting {
        major: false,
        name: "expensive_merchants",
        display: "Shuffle Expensive Merchants",
        default_display: "Shuffle Expensive Merchants Off",
        other: &[(
            "on",
            "Shuffle Expensive Merchants On",
            || collect![format!("shuffle_expensive_merchants") => json!(true)],
        )],
    },
    Setting {
        major: false,
        name: "beans_planted",
        display: "Pre-planted Magic Beans",
        default_display: "Pre-planted Magic Beans Off",
        other: &[(
            "on",
            "Pre-planted Magic Beans On",
            || collect![format!("plant_beans") => json!(true)],
        )],
    },
    Setting {
        major: false,
        name: "bombchus_in_logic",
        display: "Add Bombchu Bag and Drops",
        default_display: "Bombchu Bag and Drops Off",
        other: &[(
            "on",
            "Bombchu Bag and Drops On",
            || collect![format!("free_bombchu_drops") => json!(true)],
        )],
    },
];
...
pub(crate) fn resolve_s7_draft_settings(picks: &draft::Picks) -> seed::Settings {
    ...
    for (setting, value) in picks {
        if value != "default" {
            let Setting { other, .. } = S7_SETTINGS
                .into_iter()
                .find(|Setting { name, .. }| name == setting)
                .expect("unknown setting in draft picks");
            settings.extend(other
                .iter()
                .find(|(name, _, _)| name == value)
                .expect("unknown setting value in draft picks")
                .2());
        }
    }
    if picks
        .get("ow_tokens")
        .map(|ow_tokens| &**ow_tokens)
        .unwrap_or("default")
        == "on"
        && picks
            .get("dungeon_tokens")
            .map(|dungeon_tokens| &**dungeon_tokens)
            .unwrap_or("default")
            == "on"
    {
...
pub(crate) fn weekly_chest_appearances() -> ChestAppearances {
    static WEIGHTS: LazyLock<Vec<(ChestAppearances, usize)>> = LazyLock::new(|| {
        serde_json::from_str(include_str!("../../assets/event/s/chests-w-8.3.12.json"))
...
pub(crate) async fn info(
    ...
    Ok(match &*data.event {
        "w" => {
            ...
            let main_tournament_season =
                sqlx::query_scalar!("SELECT event FROM events WHERE series = 's'")
                    .fetch_all(&mut **transaction)
                    .await?
                    .into_iter()
                    .filter_map(|event| event.parse::<u32>().ok())
                    .max()
                    .expect("no main tournaments in database");
            let main_tournament = Data::new(
                transaction,
                Series::Standard,
                main_tournament_season.to_string(),
            )
            .await?
            .expect("database changed during transaction");
            let main_tournament_organizers = main_tournament.organizers(transaction).await?;
            let (main_tournament_organizers, race_mods) = organizers
                .into_iter()
                .partition::<Vec<_>, _>(|organizer| main_tournament_organizers.contains(organizer));
            let now = Utc::now();
            let weekly_schedules =
                WeeklySchedule::for_event(transaction, Series::Standard, "w").await?;
