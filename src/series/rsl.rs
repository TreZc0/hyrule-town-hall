...
pub(crate) enum ScriptPathError {
    ...
    #[error(transparent)]
    GitFindRemote(#[from] gix::remote::find::for_fetch::Error),
    #[error(transparent)]
...
impl VersionedPreset {
    #[cfg(unix)]
    pub(crate) fn new_unversioned(branch: &str, preset: Option<&str>) -> Result<Self, ()> {
        Ok(match branch {
            "xopar" => Self::Xopar {
                version: None,
                preset: preset
    ...
    pub(crate) fn new_versioned(
        ...
        Ok(match version.branch() {
            ootr_utils::Branch::DevR | ootr_utils::Branch::DevRob => Self::Xopar {
                version: Some(version.base().clone()),
                preset: preset
                    .map(Preset::from_str)
                    .transpose()?
                    .unwrap_or_default(),
            },
            ootr_utils::Branch::DevFenhl => Self::Fenhl {
                version: Some((version.base().clone(), version.supplementary().unwrap())),
                preset: preset
                    .map(DevFenhlPreset::from_str)
                    .transpose()?
                    .unwrap_or_default(),
    ...
    pub(crate) async fn script_path(&self) -> Result<Cow<'static, Path>, ScriptPathError> {
        let path = {
            ...
            {
                match self {
                    Self::Fenhl { version: None, .. } => Cow::Borrowed(Path::new(
                        "/opt/git/github.com/fenhl/plando-random-settings/main",
                    )),
                    Self::Fenhl {
                        version: Some((base, supplementary)),
                        ..
                    } => Cow::Owned(
                        BaseDirectories::new()
                            .find_data_file(
                                Path::new("midos-house")
                                    .join(format!("rsl-dev-fenhl-{base}-{supplementary}")),
                            )
                            .ok_or(ScriptPathError::NotFound)?,
                    ),
                    Self::Xopar { version: None, .. } | Self::XoparCustom { version: None, .. } => {
                        Cow::Owned(
                            BaseDirectories::new()
                                .find_data_file("fenhl/rslbot/plando-random-settings")
                                .ok_or(ScriptPathError::NotFound)?,
                        )
                    }
                    Self::Xopar {
                    ...
                    | Self::XoparCustom {
                        ...
                        BaseDirectories::new()
                            .find_data_file(Path::new("midos-house").join(format!("rsl-{version}")))
                            .ok_or(ScriptPathError::NotFound)?,
                    ),
                }
            }
            ...
            {
                match self {
                    Self::Fenhl { .. } => Cow::Borrowed(Path::new(
                        "C:/Users/fenhl/git/github.com/fenhl/plando-random-settings/main",
                    )), //TODO respect script version field
                    Self::Xopar { .. } | Self::XoparCustom { .. } => Cow::Borrowed(Path::new(
                        "C:/Users/fenhl/git/github.com/matthewkirby/plando-random-settings/main",
                    )), //TODO respect script version field
                }
            }
        };
        if fs::exists(&path).await? {
            ...
            if self.is_version_locked() {
                ...
                #[cfg(unix)]
                {
                    fs::create_dir_all(path.parent().expect("RSL script repo at file system root"))
                        .await?;
                    let mut cmd = Command::new("git");
                    cmd.arg("clone");
                    cmd.arg("--depth=1");
                    cmd.arg(format!(
                        "https://github.com/{}/plando-random-settings.git",
                        match self {
                    ...
                    match self {
                        Self::Xopar { .. } | Self::XoparCustom { .. } => {
                            cmd.arg("--branch=release");
                        }
                        Self::Fenhl { .. } => {}
                    }
                    ...
                    fs::copy(
                        BaseDirectories::new()
                            ...
                            .ok_or(ScriptPathError::RomPath)?,
                        rsl_data_dir.join("oot-ntscu-1.0.z64"),
                    )
                    .await?; //TODO decompress?
                }
                #[cfg(not(unix))]
                {
                    unimplemented!("clone RSL script repo on Windows")
                } //TODO
            }
        }
        Ok(path)
...
impl Leaderboard {
    pub(crate) async fn get(http_client: &reqwest::Client) -> wheel::Result<Self> {
        http_client
            .get("https://rsl.one/api/leaderboard")
            .send()
            .await?
...
pub(crate) const FORCE_OFF_SETTINGS: [ForceOffSetting; 17] = [
    ...
    ForceOffSetting {
        name: "shuffle_boulders",
        display: "Boulder Shuffle",
        lite: true,
        ban: None,
    },
    ForceOffSetting {
        name: "shuffle_individual_ocarina_notes",
        display: "Ocarina Note Shuffle",
        lite: false,
        ban: None,
    },
    ForceOffSetting {
        name: "split_collectible_bridge_conditions",
        display: "Token/Heart Bridge & GBK",
        lite: true,
        ban: Some(|weights| {
            weights.conditionals.insert(
                format!("split_collectible_bridge_conditions"),
                vec![json!(false)],
            );
        }),
    },
    ForceOffSetting {
        ...
        display: "Frog Song Rupees",
        lite: true,
        ban: None,
    },
    ForceOffSetting {
        name: "select_one_pots_crates_freestanding",
        display: "Pots & Crates & Freestanding",
        lite: false,
        ban: Some(|weights| {
            weights.conditionals.insert(
                format!("select_one_pots_crates_freestanding"),
                vec![json!(false)],
            );
        }),
    },
    ForceOffSetting {
        name: "shuffle_bosses",
        display: "Boss Door ER",
        lite: true,
        ban: Some(|weights| {
            weights
                .weights
                .get_mut("shuffle_bosses")
                .unwrap()
                .retain(|key, _| key == "off")
        }),
    },
    ForceOffSetting {
        name: "adult_trade_shuffle",
        display: "Adult Trade Shuffle",
        lite: true,
        ban: None,
    },
    ForceOffSetting {
        ...
        lite: true,
        ban: Some(|weights| {
            weights.multiselect.insert(format!("key_rings"), 0);
        }),
    },
    ForceOffSetting {
        name: "geometrically_draw_dungeon_shortcuts",
        display: "Dungeon Shortcuts",
        lite: true,
        ban: Some(|weights| {
            weights.conditionals.insert(
                format!("geometrically_draw_dungeon_shortcuts"),
                vec![json!(false)],
            );
        }),
    },
    ForceOffSetting {
        ...
        display: "Keyrings give BK",
        lite: true,
        ban: None,
    },
    ForceOffSetting {
        name: "shuffle_overworld_entrances",
        display: "Overworld ER",
        lite: false,
        ban: None,
    },
    ForceOffSetting {
        name: "mix_entrance_pools",
        display: "Mix Entrance Pools",
        lite: true,
        ban: Some(|weights| {
            weights.multiselect.insert(format!("mix_entrance_pools"), 0);
        }),
    },
    ForceOffSetting {
        name: "blue_fire_arrows",
        display: "Blue Fire Arrows",
        lite: true,
        ban: None,
    },
    ForceOffSetting {
...
pub(crate) const FIFTY_FIFTY_SETTINGS: [MultiOptionSetting; 18] = [
    MultiOptionSetting {
        ...
        display: "Complete Mask Quest",
        options: &[
            ("true", "Complete Mask Quest", true, None),
            ("false", "Complete Mask Quest Off", true, None),
        ],
    },
    MultiOptionSetting {
        name: "free_bombchu_drops",
        display: "Free Bombchu Drops",
        options: &[
            ("true", "Free Bombchu Drops", true, None),
            ("false", "Free Bombchu Drops Off", true, None),
        ],
    },
    MultiOptionSetting {
        name: "free_scarecrow",
        display: "Free Scarecrow",
        options: &[
            ("true", "Free Scarecrow", true, None),
            ("false", "Free Scarecrow Off", true, None),
        ],
    },
    MultiOptionSetting {
        name: "open_door_of_time",
        display: "Closed Door of Time",
        options: &[
            ("false", "Closed Door of Time", true, None),
            ("true", "Open Door of Time", true, None),
        ],
    },
    MultiOptionSetting {
        name: "open_forest",
        ...
        options: &[
            ("open", "Open Forest", true, None),
            ("closed_deku", "Closed Deku", true, None),
        ],
    },
    MultiOptionSetting {
        name: "owl_drops",
        display: "Owl Drops",
        options: &[
            ("true", "Owl Drops", true, None),
            ("false", "Owl Drops Off", true, None),
        ],
    },
    MultiOptionSetting {
        ...
        options: &[
            ("all", "All Locations Reachable", true, None),
            ("beatable", "Only Required Locations Reachable", true, None),
        ],
    },
    MultiOptionSetting {
        name: "ruto_already_f1_jabu",
        display: "Ruto already at F1",
        options: &[
            ("true", "Ruto already at F1", true, None),
            ("false", "Ruto already at F1 Off", true, None),
        ],
    },
    MultiOptionSetting {
        name: "shuffle_beans",
        ...
        options: &[
            ("true", "Shuffle Beans", true, None),
            ("false", "Shuffle Beans Off", true, None),
        ],
    },
    MultiOptionSetting {
        name: "shuffle_cows",
        ...
        options: &[
            ("true", "Shuffle Cows", true, None),
            ("false", "Shuffle Cows Off", true, None),
        ],
    },
    MultiOptionSetting {
        ...
        options: &[
            ("true", "Shuffle Gerudo Card", true, None),
            ("false", "Shuffle Gerudo Card Off", true, None),
        ],
    },
    MultiOptionSetting {
        ...
        options: &[
            ("true", "Shuffle Kokiri Sword", true, None),
            ("false", "Shuffle Kokiri Sword Off", false, None),
        ],
    },
    MultiOptionSetting {
        name: "shuffle_ocarinas",
        display: "Shuffle Ocarina",
        options: &[
            ("true", "Shuffle Ocarina", true, None),
            ("false", "Shuffle Ocarina Off", true, None),
        ],
    },
    MultiOptionSetting {
        name: "start_with_consumables",
        display: "Start with Consumables",
        options: &[
            ("true", "Start with Consumables", true, None),
            ("false", "Start with Consumables Off", true, None),
        ],
    },
    MultiOptionSetting {
        name: "start_with_rupees",
        display: "Start with Rupees",
        options: &[
            ("true", "Start with Rupees", true, None),
            ("false", "Start with Rupees Off", true, None),
        ],
    },
    MultiOptionSetting {
        name: "warp_songs",
        display: "Random Warp Songs",
        options: &[
            ("true", "Random Warp Songs", true, None),
            ("false", "Random Warp Songs Off", true, None),
        ],
    },
    MultiOptionSetting {
        name: "zora_fountain",
        ...
        options: &[
            ("open", "Open Fountain", true, None),
            ("closed", "Closed Fountain", true, None),
        ],
    },
    MultiOptionSetting {
        ...
        display: "Grotto ER",
        options: &[
            ("true", "Grotto ER", true, None),
            ("false", "Grotto ER Off", true, None),
        ],
    },
];
...
pub(crate) const MULTI_OPTION_SETTINGS: [MultiOptionSetting; 19] = [
    MultiOptionSetting {
        ...
        options: &[("open", "Open", true, None)],
    },
    MultiOptionSetting {
        name: "shuffle_ganon_bosskey",
        display: "Ganon BK",
        options: &[("remove", "Remove", true, None)],
    },
    MultiOptionSetting {
        name: "correct_chest_appearances",
        display: "Correct Chest Appearance",
        options: &[
            ("textures", "Textures", false, None),
            ("both", "Both", true, None),
            ("off", "Off", true, None),
        ],
    },
    MultiOptionSetting {
        ...
        options: &[
            ("quadruple", "Quadruple", false, None),
            ("double", "Double", true, None),
            ("normal", "Normal", true, None),
            ("half", "Half", true, None),
        ],
    },
    MultiOptionSetting {
        name: "gerudo_fortress",
        display: "Gerudo Fortress Guards",
        options: &[
            ("open", "Open", true, None),
            ("fast", "Fast", true, None),
            ("normal", "Normal", true, None),
    ...
    MultiOptionSetting {
        ...
        display: "Item Pool",
        options: &[
            ("minimal", "Minimal", false, None),
            ("scarce", "Scarce", true, None),
            ("balanced", "Balanced", true, None),
            ("plentiful", "Plentiful", true, None),
        ],
    },
    MultiOptionSetting {
        ...
        options: &[
            (
                ...
                Some(|weights| {
                    let weight = weights
                        .weights
                        .get_mut("shuffle_dungeon_entrances")
                        .unwrap();
                    weight.remove("all");
                    weight.remove("simple");
                }),
            ),
            (
                "all_off",
        ...
        options: &[
            (
                ...
                Some(|weights| {
                    let weight = weights
                        .weights
                        .get_mut("shuffle_interior_entrances")
                        .unwrap();
                    weight.remove("all");
                    weight.remove("simple");
                }),
    ...
    MultiOptionSetting {
        ...
        options: &[
            (
                ...
                true,
                Some(|weights| {
                    let weight = weights.weights.get_mut("shopsanity").unwrap();
                    weight.remove("off");
                    weight.remove("0");
                    weight.remove("1");
                }),
    ...
    MultiOptionSetting {
        ...
        options: &[
            (
                ...
                true,
                Some(|weights| {
                    let weight = weights.weights.get_mut("shopsanity_prices").unwrap();
                    weight.remove("random_giant");
                    weight.remove("random");
                }),
            ),
            (
                "adult",
            ...
            (
                ...
                Some(|weights| {
                    weights
                        ...
                        .get_mut("shopsanity_prices")
                        .unwrap()
                        .remove("random_starting");
                }),
            ),
            ("affordable", "Affordable", true, None),
        ],
    },
    MultiOptionSetting {
        name: "shuffle_scrubs",
        display: "Scrub Shuffle",
        options: &[
            (
                "random_regular",
                "Random + Regular",
                false,
                Some(|weights| {
                    let weight = weights.weights.get_mut("shuffle_scrubs").unwrap();
                    weight.remove("random");
                    weight.remove("regular");
                }),
            ),
            ("low", "Low", true, None),
            ("off", "Off", true, None),
    ...
    MultiOptionSetting {
        ...
        options: &[
            (
                ...
                true,
                Some(|weights| {
                    let weight = weights.weights.get_mut("tokensanity").unwrap();
                    weight.remove("all");
                    weight.remove("dungeons");
                }),
            ),
            (
                "all_overworld",
    ...
    MultiOptionSetting {
        ...
        display: "Treasure Chest Game Keys",
        options: &[
            ("keysanity", "Keysanity", true, None),
            (
                "keysanity_regional",
                "Keysanity + Regional",
                true,
                Some(|weights| {
                    let weight = weights.weights.get_mut("shuffle_tcgkeys").unwrap();
                    weight.remove("keysanity");
                    weight.remove("regional");
                }),
            ),
        ],
    },
    MultiOptionSetting {
        ...
        display: "Song Shuffle",
        options: &[
            ("dungeon", "Songs On Dungeon", true, None),
            ("any", "Anywhere", true, None),
            ("song", "Songs on songs", true, None),
        ],
    },
    MultiOptionSetting {
        name: "shuffle_enemy_spawns",
        display: "Enemy Souls",
        options: &[
            ("all", "All", false, None),
            (
                ...
                true,
                Some(|weights| {
                    let weight = weights.weights.get_mut("shuffle_enemy_spawns").unwrap();
                    weight.remove("all");
                    weight.remove("regional");
                    weight.remove("bosses");
                }),
            ),
        ],
    },
    MultiOptionSetting {
        ...
        options: &[
            (
                "dungeon_vanilla_remove",
                "Own Dungeon + Vanilla + Remove",
                true,
                Some(|weights| {
                    let weight = weights.weights.get_mut("shuffle_smallkeys").unwrap();
                    weight.remove("dungeon");
                    weight.remove("vanilla");
                    weight.remove("remove");
                }),
            ),
            ("regional", "Regional", true, None),
            (
                "anydungeon_overworld_keysanity",
                "Any Dungeon + Overworld + Keysanity",
                true,
                Some(|weights| {
                    let weight = weights.weights.get_mut("shuffle_smallkeys").unwrap();
                    weight.remove("any_dungeon");
                    weight.remove("overworld");
                    weight.remove("keysanity");
                }),
            ),
    ...
    MultiOptionSetting {
        ...
        options: &[
            ...
            ("regional", "Regional", true, None),
            ("keysanity", "Keysanity", false, None),
        ],
    },
    MultiOptionSetting {
        name: "shuffle_silver_rupees",
        display: "Silver Rupees Shuffle",
        options: &[
            ("anywhere", "Anywhere", true, None),
            (
                "anywhere_dungeon_regional_remove",
                "Anywhere + Dungeon + Regional + Remove",
                true,
                Some(|weights| {
                    let weight = weights.weights.get_mut("shuffle_silver_rupees").unwrap();
                    weight.remove("anywhere");
                    weight.remove("dungeon");
                    weight.remove("regional");
                    weight.remove("remove");
                }),
        ...
        options: &[
            (
                ...
                Some(|weights| {
                    weights
                        .weights
                        .insert(format!("trials_random"), collect![format!("false") => 1]);
                    let weight = weights.weights.get_mut("trials").unwrap();
                    weight.remove("0");
                    weight.remove("1");
                }),
            ),
    ...
    format!(
        ...
        English
            .join_str_opt(
                FORCE_OFF_SETTINGS
                    .into_iter()
                    .filter(|ForceOffSetting { name, .. }| picks
                        .get(*name)
                        .is_some_and(|pick| pick == "banned"))
                    .map(|ForceOffSetting { display, .. }| Cow::Borrowed(display))
                    .chain(
                        FIFTY_FIFTY_SETTINGS
                            .into_iter()
                            .chain(MULTI_OPTION_SETTINGS)
                            .flat_map(
                                |MultiOptionSetting {
                                     name,
                                     options,
                                    ...
                                    .into_iter()
                                    .filter(|(option_name, _, _, _)| picks.get(name).is_some_and(
                                        |pick| pick.split(',').any(|pick| pick == *option_name)
                                    ))
                                    .map(move |(_, option_display, _, _)| Cow::Owned(format!(
                                        "{display}: {option_display}"
                                    )))
                            )
                    )
            )
            .map(|bans| format!("the following bans: {bans}"))
            .unwrap_or_else(|| format!("no bans"))
    )
}

pub(crate) async fn resolve_s7_draft_weights(
    script_path: &Path,
    picks: &draft::Picks,
) -> Result<Weights, draft::Error> {
    let is_lite = picks
        .get("preset")
        .map(|preset| &**preset)
        .unwrap_or("league")
        == "lite";
    let mut weights =
        fs::read_json::<Weights>(script_path.join("weights").join("rsl_season7.json")).await?;
    weights.weights.insert(
        format!("password_lock"),
        collect![format!("true") => 1, format!("false") => 0],
    );
    if is_lite {
        let ovr =
            fs::read_json::<Weights>(script_path.join("weights").join("beginner_override.json"))
                .await?;
        for (option, value) in ovr.options {
            if let Some(option) = option.strip_prefix("extra_") {
                ...
                if let Some(entry) = weights.options.get_mut(option) {
                    ...
                    entry
                        .as_array_mut()
                        .ok_or(draft::Error::RslRemoveType)?
                        .retain(|item| !value.contains(item));
                }
            } else {
    ...
    for MultiOptionSetting { name, options, .. } in FIFTY_FIFTY_SETTINGS
        .into_iter()
        .chain(MULTI_OPTION_SETTINGS)
    {
        for (option, _, _, ban) in options {
            if picks
                .get(name)
                .is_some_and(|pick| pick.split(',').contains(option))
            {
                if let Some(ban) = ban {
                    ban(&mut weights);
                } else {
