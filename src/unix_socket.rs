                        ...
                        match ClientMessage::read(&mut sock).await {
                            ...
                            Ok(ClientMessage::Seed { is_official, spoiler_seed, no_password, no_web, goal, args }) => {
                                ...
                                let mut rx = match goal.parse_seed_command(&mut transaction, &global_state, is_official, spoiler_seed, no_password, &args).await {
                                    ...
                                    Ok(SeedCommandParseResult::Twwr { permalink, unlock_spoiler_log, description, .. }) => {
                                        Some(SeedRollUpdate::Message(description)).write(&mut sock).await.expect("error writing to UNIX socket");
                                        global_state.clone().roll_twwr_seed(None, permalink, unlock_spoiler_log)
                                    }
                                    Ok(SeedCommandParseResult::QueueExisting { data, description, .. }) => {
                                        Some(SeedRollUpdate::Message(description)).write(&mut sock).await.expect("error writing to UNIX socket");
                                        Some(SeedRollUpdate::Done { rsl_preset: None, version: None, unlock_spoiler_log: UnlockSpoilerLog::After, seed: data, resolved_randoms: None }).write(&mut sock).await.expect("error writing to UNIX socket");
                                        None::<SeedRollUpdate>.write(&mut sock).await.expect("error writing to UNIX socket");
