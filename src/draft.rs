use {
    crate::{
        event::teams::{self, SignupsTeam},
        prelude::*,
    },
    std::cmp::{max_by_key, min_by_key},
};
...
impl Kind {
    pub(crate) fn game_count(&self) -> Option<usize> {
        match self {
            Self::PickOnly {
                picks_per_player, ..
            } => Some(usize::from(*picks_per_player) * 2),
            Self::BanPick { order, .. } => Some(
                order
                    .iter()
                    .filter(|p| matches!(p, DraftPhase::Pick(_)))
                    .count()
                    + 1,
            ),
            Self::BanOnly { options, order, .. } => options.len().checked_sub(order.len()),
            _ => None,
        }
    }

    pub(crate) fn validate(&self) -> Result<(), String> {
        let (options, label) = match self {
            Self::PickOnly { options, label, .. }
            | Self::BanPick { options, label, .. }
            | Self::BanOnly { options, label, .. } => (options, label),
            _ => return Ok(()),
        };
        if options.is_empty() || label.trim().is_empty() {
            return Err("A draft requires options and a non-empty label.".into());
        }
        let mut keys = HashSet::new();
        for option in options {
            if option.display_name.trim().is_empty()
                || option.preset.trim().is_empty()
                || !keys.insert(&option.preset)
            {
                return Err(
                    "Draft options require non-empty names and unique preset identifiers.".into(),
                );
            }
        }
        let games = self.game_count().unwrap_or(0);
        if !(1..=255).contains(&games) {
            return Err("A draft must assign between 1 and 255 games.".into());
        }
        match self {
            Self::PickOnly { unique, .. } if *unique && options.len() < games => {
                return Err("Not enough unique options for the configured picks.".into());
            }
            Self::BanPick { order, .. } => {
                if !matches!(order.last(), Some(DraftPhase::Pick(_)))
                    || options.len() != order.len() + 1
                {
                    return Err("Ban/pick drafts must end with a pick and leave exactly one unselected decider preset.".into());
                }
            }
            Self::BanOnly { order, .. } => {
                if order.is_empty() || order.iter().any(|p| matches!(p, DraftPhase::Pick(_))) {
                    return Err(
                        "Ban-only drafts require a non-empty order containing only bans.".into(),
                    );
                }
            }
            _ => {}
    ...
    pub(crate) fn uses_button_draft(&self) -> bool {
        matches!(
            self,
            Self::PickOnly { .. } | Self::BanPick { .. } | Self::BanOnly { .. }
        )
    }

    pub(crate) fn preset_display_name(&self, preset: &str) -> Option<&str> {
        let options: &[PresetOption] = match self {
            Self::PickOnly { options, .. }
            | Self::BanPick { options, .. }
            | Self::BanOnly { options, .. } => options,
            _ => return None,
        };
        options
            .iter()
    ...
    pub(crate) fn from_db(
        ...
        match kind_str {
            "s7" => return Some(Self::S7),
            "multiworld_s3" => return Some(Self::MultiworldS3),
            "multiworld_s4" => return Some(Self::MultiworldS4),
            "multiworld_s5" => return Some(Self::MultiworldS5),
            "rsl_s7" => return Some(Self::RslS7),
            "tournoifranco_s3" => return Some(Self::TournoiFrancoS3),
            "tournoifranco_s4" => return Some(Self::TournoiFrancoS4),
            "tournoifranco_s5" => return Some(Self::TournoiFrancoS5),
            _ => {}
        }
        let config = config?;
        match kind_str {
            "ban_only" => Some(Self::BanOnly {
                options: parse_preset_options(config)?,
                order: parse_draft_order(config)?,
                label: parse_label(config)?,
            }),
            "pick_only" => Some(Self::PickOnly {
                options: parse_preset_options(config)?,
                who_starts: parse_who_starts(config)?,
                picks_per_player: parse_picks_per_player(config)?,
                unique: parse_unique(config)?,
                label: parse_label(config)?,
            }),
            "ban_pick" => Some(Self::BanPick {
                options: parse_preset_options(config)?,
                order: parse_draft_order(config)?,
                label: parse_label(config)?,
            }),
...
fn parse_team(s: &str) -> Option<Team> {
    match s {
        "high_seed" => Some(Team::HighSeed),
        "low_seed" => Some(Team::LowSeed),
        _ => None,
    }
}

fn parse_draft_order(config: &serde_json::Value) -> Option<Vec<DraftPhase>> {
    let rows: Vec<DraftPhaseRow> = serde_json::from_value(config.get("order")?.clone()).ok()?;
    rows.into_iter()
        .map(|row| {
            let team = parse_team(&row.team)?;
            match &*row.phase {
                "ban" => Some(DraftPhase::Ban(team)),
                "pick" => Some(DraftPhase::Pick(team)),
                _ => None,
            }
        })
        .collect()
...
fn parse_who_starts(config: &serde_json::Value) -> Option<Team> {
    parse_team(config.get("who_starts")?.as_str()?)
}

fn parse_preset_options(config: &serde_json::Value) -> Option<Vec<PresetOption>> {
    serde_json::from_value(config.get("options")?.clone()).ok()
}

fn parse_label(config: &serde_json::Value) -> Option<String> {
    config.get("label")?.as_str().map(String::from)
}

fn parse_picks_per_player(config: &serde_json::Value) -> Option<u8> {
    config
        .get("picks_per_player")?
        .as_u64()
        .and_then(|n| n.try_into().ok())
}

fn parse_unique(config: &serde_json::Value) -> Option<bool> {
    config.get("unique")?.as_bool()
}

#[cfg(test)]
mod configured_draft_tests {
    use super::*;

    fn options(count: usize) -> Vec<PresetOption> {
        (0..count)
            .map(|n| PresetOption {
                display_name: format!("Preset {n}"),
                preset: format!("preset{n}"),
            })
            .collect()
    }

    async fn complete(kind: &Kind) -> Draft {
        ...
        for _ in 0..32 {
            let step = draft
                .next_step(kind, Some(1), &mut MessageContext::None)
                .await
                .unwrap();
            let action = match step.kind {
                StepKind::Ban {
                    ...
                    setting: available_settings.all().next().unwrap().name.to_string(),
                },
                StepKind::PickPreset {
                    available_presets, ..
                } => Action::Pick {
                    setting: available_presets[0].preset.clone(),
                    value: String::new(),
                },
                StepKind::Done(_) => return draft,
                _ => panic!("unexpected generic draft step"),
            };
            draft
                .apply(kind, Some(1), &mut MessageContext::None, action)
                .await
                .unwrap()
                .unwrap();
            // Exercise the existing database representation between every action.
            draft = serde_json::from_value(serde_json::to_value(&draft).unwrap()).unwrap();
        }
        panic!("draft did not complete");
    }

    #[tokio::test]
    async fn ban_pick_preserves_three_games_and_supports_five() {
        for picks in [2, 4] {
            let mut order = vec![
                DraftPhase::Ban(Team::LowSeed),
                DraftPhase::Ban(Team::HighSeed),
            ];
            order.extend((0..picks).map(|n| {
                DraftPhase::Pick(if n % 2 == 0 {
                    Team::HighSeed
                } else {
                    Team::LowSeed
                })
            }));
            let kind = Kind::BanPick {
                options: options(picks + 3),
                order,
                label: "preset".into(),
            };
            let draft = complete(&kind).await;
            for game in 1..=picks + 1 {
                let step = draft
                    .next_step(&kind, Some(game as i16), &mut MessageContext::None)
                    .await
                    .unwrap();
                let StepKind::Done(settings) = step.kind else {
                    panic!("game is not ready")
                };
                assert_eq!(settings["preset"], format!("preset{}", game + 1));
            }
        }
    }

    #[tokio::test]
    async fn ban_only_assigns_every_remaining_preset_for_any_match_length() {
        for games in [1, 2, 3, 5] {
            let kind = Kind::BanOnly {
                options: options(games + 2),
                order: vec![
                    DraftPhase::Ban(Team::HighSeed),
                    DraftPhase::Ban(Team::LowSeed),
                ],
            ...
            for game in 1..=games {
                ...
                assert!(assigned.insert(settings["preset"].as_str().unwrap().to_owned()));
            }
            assert!(!assigned.contains("preset0") && !assigned.contains("preset1"));
        }
    }

    #[test]
    fn rejects_impossible_action_sequences() {
        assert!(
            Kind::BanOnly {
                options: options(2),
                order: vec![DraftPhase::Pick(Team::HighSeed)],
                label: "preset".into()
            }
            .validate()
            .is_err()
        ...
        assert!(
            Kind::BanPick {
                options: options(2),
                order: vec![
                    DraftPhase::Pick(Team::HighSeed),
                    DraftPhase::Pick(Team::LowSeed)
                ],
                label: "preset".into()
            }
            .validate()
...
pub(crate) struct BanSetting {
    ...
    pub(crate) default_display: Cow<'static, str>,
    pub(crate) description: Cow<'static, str>,
}

pub(crate) struct BanSettings(Vec<(Cow<'static, str>, Vec<BanSetting>)>);
...
impl BanSettings {
    ...
    pub(crate) fn page(&self, idx: usize) -> Option<(&str, &[BanSetting])> {
        self.0
            .get(idx)
...
impl<'a> MessageContext<'a> {
    ...
    pub(crate) fn into_transaction(self) -> Transaction<'a, Postgres> {
        let Self::Discord { transaction, .. } = self else {
            panic!("called into_transaction on non-Discord draft message context")
        };
        transaction
    }
}
...
pub(crate) enum Action {
    GoFirst(bool),
    Ban { setting: String },
    Pick { setting: String, value: String },
    Skip,
    BooleanChoice(bool),
}
...
impl Draft {
    pub(crate) async fn for_game1(
        transaction: &mut Transaction<'_, Postgres>,
        http_client: &reqwest::Client,
        kind: &Kind,
        event: &event::Data<'_>,
        phase: Option<&str>,
        [team1, team2]: [&team::Team; 2],
    ) -> Result<Self, cal::Error> {
        let [high_seed, low_seed] = match kind {
            ...
            Kind::MultiworldS3 | Kind::MultiworldS4 | Kind::MultiworldS5 => {
                if phase.is_some_and(|phase| phase == "Top 8") {
                    ...
                    team_ids.sort_unstable_by_key(|team| {
                        seeding.iter().position(|iter_team| iter_team == team)
                    });
                    team_ids
                } else {
                    ...
                    let signups = teams::signups_sorted(
                        &mut *transaction,
                        &mut teams::Cache::new(http_client.clone()),
                        None,
                        event,
                        false,
                        qualifier_kind,
                    ...
                    let SignupsTeam {
                        ...
                        .iter()
                        .find(|SignupsTeam { team, .. }| {
                            team.as_ref().is_some_and(|team| team == team1)
                        })
                        .expect("match with team that didn't sign up");
                    let SignupsTeam {
                        members: members2, ..
                    } = signups
                        .iter()
                        .find(|SignupsTeam { team, .. }| {
                            team.as_ref().is_some_and(|team| team == team2)
                        })
                        .expect("match with team that didn't sign up");
                    let avg1 = members1
                        .iter()
                        .try_fold(Duration::default(), |acc, member| {
                            Some(acc + member.qualifier_time?)
                        })
                        .map(|total| {
                            total / u32::try_from(members1.len()).expect("too many team members")
                        });
                    let avg2 = members2
                        .iter()
                        .try_fold(Duration::default(), |acc, member| {
                        ...
                        .map(|total| {
                            total / u32::try_from(members2.len()).expect("too many team members")
                        });
                    match [avg1, avg2] {
    ...
    pub(crate) async fn for_next_game(
        transaction: &mut Transaction<'_, Postgres>,
        kind: &Kind,
        loser: Id<Teams>,
        winner: Id<Teams>,
        ...
        Ok(Self {
            ...
            settings: match kind {
                ...
                Kind::TournoiFrancoS3 | Kind::TournoiFrancoS4 | Kind::TournoiFrancoS5 => {
                    let team_rows = sqlx::query!(
                        "SELECT hard_settings_ok, mq_ok FROM teams WHERE id = $1 OR id = $2",
                        loser as _,
                        winner as _
                    )
                    .fetch_all(&mut **transaction)
    ...
    fn pick_count(&self, kind: &Kind) -> u8 {
        match kind {
            Kind::PickOnly {
                ...
                u8::try_from(
                    (1..=total)
                        .filter(|&n| self.settings.contains_key(&*format!("game{n}_preset")))
                        .count(),
                )
                .unwrap()
            }
            ...
            Kind::RslS7 => {
                self.skipped_bans
                    + u8::try_from(
                        rsl::FORCE_OFF_SETTINGS
                            .into_iter()
                            .filter(|&rsl::ForceOffSetting { name, .. }| {
                    ...
                    + u8::try_from(
                        rsl::FIFTY_FIFTY_SETTINGS
                            ...
                            .filter(|&rsl::MultiOptionSetting { name, .. }| {
                                self.settings.contains_key(name)
                            })
                            .count(),
                    )
    ...
    async fn next_step_s7(
        &self,
        game: Option<i16>,
        msg_ctx: &mut MessageContext<'_>,
    ) -> Result<Step, Error> {
        let kind = Kind::S7;
        Ok({
            if let Some(went_first) = self.went_first {
                match self.pick_count(&kind) {
                    prev_bans @ 0..=1 => {
                        ...
                        let (major_setings, minor_settings) = s::S7_SETTINGS
                            .into_iter()
                            .partition::<Vec<_>, _>(|&s::Setting { major, .. }| major);
                        Step {
                            kind: StepKind::Ban {
                                available_settings: BanSettings(vec![
                                    ("Major Settings".into(), major_setings.into_iter()
                                        ...
                                        .map(|setting @ s::Setting { name, display, default_display, .. }| BanSetting {
                                            ...
                                            name: name.into(), display: display.into(), default_display: default_display.into(),
                                        })
                                        .collect()),
                                    ("Minor Settings".into(), minor_settings.into_iter()
                                        .filter(|&s::Setting { name, .. }| !self.settings.contains_key(name))
                                        .map(|setting @ s::Setting { name, display, default_display, .. }| BanSetting {
                                            default: "default".into(),
                                            description: Cow::Owned(setting.description()),
                        ...
                        Step {
                            kind: StepKind::Pick {
                                available_choices: DraftSettings(vec![(
                                    ...
                                    s::S7_SETTINGS
                                        .into_iter()
                                        .filter(|&s::Setting { name, major, .. }| {
                                            major == (n < 4) && !self.settings.contains_key(name)
                                        })
                                        .map(
                                            |setting @ s::Setting {
                                                 name,
                            ...
                            message: match msg_ctx {
                                ...
                                MessageContext::Discord {
                                    ...
                                    let (mut high_seed, mut low_seed) = teams
                                        .iter()
                                        .partition::<Vec<_>, _>(|team| team.id == self.high_seed);
                                    let high_seed = high_seed.remove(0);
                                    let low_seed = low_seed.remove(0);
                                    match n {
                                        2 | 3 => MessageBuilder::default()
                                            .mention_team(
                                                transaction,
                                                Some(*guild_id),
                                                team.choose(high_seed, low_seed),
                                            )
                                            .await?
                                ...
                                MessageContext::RaceTime {
                                    high_seed_name,
                                    low_seed_name,
                                    ..
                                } => match n {
                                    2 => format!(
                                        "{}, pick a major setting using “!pick <setting> <value>”",
                                        team.choose(high_seed_name, low_seed_name)
                                    ),
                                    3 => format!(
                                        "{}, pick a major setting.",
                                        team.choose(high_seed_name, low_seed_name)
                                    ),
                                    4 | 5 => format!(
                                        "{}, pick a minor setting.",
                                        team.choose(high_seed_name, low_seed_name)
                                    ),
                                    0..=1 | 6.. => unreachable!(),
                                },
                    ...
                    6.. => Step {
                        ...
                        message: match msg_ctx {
                            ...
                            MessageContext::Discord { .. } => format!(
                                "Settings draft completed. You will be playing with {}.",
                                s::display_s7_draft_picks(&self.settings)
                            ),
                            MessageContext::RaceTime { .. } => {
                                s::display_s7_draft_picks(&self.settings)
                            }
                ...
                Step {
                    ...
                    message: match msg_ctx {
                        ...
                        MessageContext::Discord {
                            ...
                            let (mut high_seed, _) = teams
                                .iter()
                                .partition::<Vec<_>, _>(|team| team.id == self.high_seed);
                            let high_seed = high_seed.remove(0);
                            let mut builder = MessageBuilder::default();
                            builder
                                .mention_team(transaction, Some(*guild_id), high_seed)
                                .await?;
                            if game.is_some_and(|game| game > 1) {
                                builder.push(": as the loser of the previous race, please choose whether you want to go ");
                            } else {
                                builder.push(
                                    ": you have the higher seed. Choose whether you want to go ",
                                );
                            }
                        ...
                        MessageContext::RaceTime { high_seed_name, .. } => format!(
                            "{high_seed_name}, you have the higher seed. Choose whether you want to go !first or !second"
                        ),
                    },
                }
            }
            ...
            if let Some(went_first) = self.went_first {
                ...
                match (is_lite, self.pick_count(&kind)) {
                    (true, n @ (0 | 1)) | (false, n @ (0 | 1 | 4 | 5)) => {
                        ...
                        let rsl_script_path = rsl::VersionedPreset::XoparCustom {
                            version: None,
                            weights: rsl::Weights::default(),
                        }
                        .script_path()
                        .await?;
                        for setting in rsl::MULTI_OPTION_SETTINGS
                            .into_iter()
                            // Weights in blocked settings may not be banned
                            .filter(|&rsl::MultiOptionSetting { name, .. }| {
                                self.settings
                                    .get(name)
                                    .is_none_or(|value| value != "blocked")
                            })
                            // Each player may only ban one weight within each setting
                            .filter(|&rsl::MultiOptionSetting { name, .. }| {
                                self.settings.get(&*format!("{name}_banned_by")).is_none_or(
                                    |banned_by| {
                                        !banned_by
                                            .split(',')
                                            .any(|banned_by| banned_by == team.to_string())
                                    },
                                )
                            })
                        {
                            ...
                            for (name, display, lite, ban) in setting.options {
                                ...
                                let mut weights =
                                    rsl::resolve_s7_draft_weights(&rsl_script_path, &self.settings)
                                        .await?;
                                if let Some(ban) = ban {
                                    ban(&mut weights);
                                } else {
                                ...
                                if weights
                                    .weights
                                    .into_values()
                                    .all(|weight| weight.into_values().any(|value| value > 0))
                                {
                            ...
                            if let Ok(options) = NEVec::try_from(options) {
                                multi_options_settings.push(DraftSetting {
                                    ...
                                    options: options
                                        .iter()
                                        .map(|(name, display)| DraftSettingChoice {
                                            name,
                                            display: format!("{}: {display}", setting.display)
                                                .into(),
                                        })
                                        .collect(),
                                    description: Cow::Owned(format!(
                                        ...
                                        English.join_str_with(
                                            "or",
                                            options.into_nonempty_iter().map(|(name, _)| name)
                                        )
                                    )),
                                });
                            }
                    ...
                    (false, n @ (2 | 3)) => {
                        ...
                        Step {
                            kind: StepKind::Ban {
                                available_settings: BanSettings(vec![
                                    ("“Force Off” Settings".into(), rsl::FORCE_OFF_SETTINGS.into_iter()
                                        .filter(|&rsl::ForceOffSetting { name, lite, .. }| !self.settings.contains_key(name) && (!is_lite || lite))
                                        .map(|rsl::ForceOffSetting { name, display, .. }|
                                            BanSetting {
                                                default: "blocked".into(),
                                                default_display: display.into(),
                                                description: Cow::Owned(format!("{name}: blocked")),
                                                name: name.into(), display: display.into(),
                                            }
                                        )
                                        .collect()),
                                    ("“50/50” Settings".into(), rsl::FIFTY_FIFTY_SETTINGS.into_iter()
                                        .filter(|&rsl::MultiOptionSetting { name, options, .. }| !self.settings.contains_key(name) && (!is_lite || options.iter().any(|(_, _, lite, _)| *lite)))
                                        ...
                                        .collect()),
                                    ("“Multiple Options” Settings".into(), rsl::MULTI_OPTION_SETTINGS.into_iter()
                                        .filter(|&rsl::MultiOptionSetting { name, options, .. }| !self.settings.contains_key(name) && (!is_lite || options.iter().any(|(_, _, lite, _)| *lite)))
                                        .map(|rsl::MultiOptionSetting { name, display, .. }|
                                            BanSetting {
                                                default: "blocked".into(),
                    ...
                    (true, 2..) | (false, 6..) => Step {
                        kind: StepKind::DoneRsl {
                            preset: rsl::VersionedPreset::XoparCustom {
                                ...
                                weights: rsl::resolve_s7_draft_weights(
                                    &rsl::VersionedPreset::XoparCustom {
                                        version: None,
                                        weights: rsl::Weights::default(),
                                    }
                                    .script_path()
                        ...
                        message: match msg_ctx {
                            MessageContext::None => String::default(),
                            MessageContext::Discord { .. } => format!(
                                "Weights draft completed. You will be playing with {}.",
                                rsl::display_s7_draft_picks(&self.settings)
                            ),
                            MessageContext::RaceTime { .. } => {
                                rsl::display_s7_draft_picks(&self.settings)
                            }
    ...
    async fn next_step_multiworld_s3(
        &self,
        game: Option<i16>,
        msg_ctx: &mut MessageContext<'_>,
    ) -> Result<Step, Error> {
                ...
                match self.pick_count(&kind) {
                        ...
                        Step {
                            ...
                            message: match msg_ctx {
                                ...
                                MessageContext::RaceTime {
                                    ...
                                    team.choose(high_seed_name, low_seed_name),
                                    if prev_bans == 0 {
                                        " Use “!settings” for a list of available settings."
                                    } else {
                                        ""
                                    },
                                ),
                        ...
                        Step {
                                    ...
                                    mw::S3_SETTINGS
                                        ...
                                        .map(
                                            |mw::Setting {
                                                ...
                                                DraftSetting {
                                                    options: iter::once(DraftSettingChoice {
                                                        name: default,
                                                        display: default_display.into(),
                                                    })
                                                    .chain(other.iter().map(|&(name, display)| {
                                                        DraftSettingChoice {
                                                            name,
                                                            display: display.into(),
                                                        }
                            ...
                            message: match msg_ctx {
                                ...
                                MessageContext::RaceTime {
                                    ...
                                    2 => format!(
                                        "{}, pick a setting using “!pick <setting> <value>”",
                                        team.choose(high_seed_name, low_seed_name)
                                    ),
                                    3 => format!(
                                        "{}, pick two settings.",
                                    ...
                                    5 => format!(
                                        "{}, pick the final setting. You can also use “!skip” if you want to leave the settings as they are.",
                                        team.choose(high_seed_name, low_seed_name)
                                    ),
                                    0..=1 | 6.. => unreachable!(),
                                },
                    ...
                    6.. => Step {
                        ...
                        message: match msg_ctx {
                            ...
                            MessageContext::Discord { .. } => format!(
                                "Settings draft completed. You will be playing with {}.",
                                mw::display_s3_draft_picks(&self.settings)
                            ),
                            MessageContext::RaceTime { .. } => {
                                mw::display_s3_draft_picks(&self.settings)
                            }
    ...
    async fn next_step_multiworld_s4(
        &self,
        game: Option<i16>,
        msg_ctx: &mut MessageContext<'_>,
    ) -> Result<Step, Error> {
                ...
                match self.pick_count(&kind) {
                    prev_bans @ (0..=1 | 6..=7) => {
                        ...
                        Step {
                            kind: StepKind::Ban {
                                available_settings: BanSettings(vec![
                                    ("All Settings".into(), mw::S4_SETTINGS.iter().copied()
                                        .filter(|&mw::Setting { name, .. }| !self.settings.contains_key(name))
                                        .map(|mw::Setting { name, display, default, default_display, description, .. }|
                                            if name == "camc" && self.settings.get("special_csmc").map(|special_csmc| &**special_csmc).unwrap_or("no") == "yes" {
                                                BanSetting {
                                                    default: "both".into(),
                                                    default_display: "chest size & texture match contents".into(),
                                                ...
                                                BanSetting {
                                                    description: Cow::Borrowed(description),
                                                    name: name.into(), display: display.into(), default: default.into(), default_display: default_display.into(),
                                                }
                                            }
                                        )
                                        .collect()),
                    ...
                    n @ (2..=5 | 8..=9) => {
                        let team = match (n, went_first) {
                            ...
                            | (5, true)
                            | (8, false)
                            | (9, true) => Team::HighSeed,
                            (2, false)
                            | (3, true)
                            | (4, true)
                            | (5, false)
                            | (8, true)
                            | (9, false) => Team::LowSeed,
                    ...
                    10.. => Step {
                        ...
                        message: match msg_ctx {
                            ...
                            MessageContext::Discord { .. } => format!(
                                "Settings draft completed. You will be playing with {}.",
                                mw::display_s4_draft_picks(&self.settings)
                            ),
                            MessageContext::RaceTime { .. } => {
                                mw::display_s4_draft_picks(&self.settings)
                            }
    ...
    async fn next_step_multiworld_s5(
        &self,
        game: Option<i16>,
        msg_ctx: &mut MessageContext<'_>,
    ) -> Result<Step, Error> {
                ...
                match self.pick_count(&kind) {
                    ...
                    10.. => Step {
                        ...
                        message: match msg_ctx {
                            ...
                            MessageContext::Discord { .. } => format!(
                                "Settings draft completed. You will be playing with {}.",
                                mw::display_s5_draft_picks(&self.settings)
                            ),
                            MessageContext::RaceTime { .. } => {
                                mw::display_s5_draft_picks(&self.settings)
                            }
    ...
    async fn next_step_tournoi_franco(
        &self,
        kind: &Kind,
        _game: Option<i16>,
        msg_ctx: &mut MessageContext<'_>,
        ...
        let all_settings = match kind {
            ...
            | Kind::PickOnly { .. }
            | Kind::BanPick { .. }
            | Kind::BanOnly { .. } => unreachable!(),
        };
        Ok({
            if let Some(went_first) = self.went_first {
                let mut pick_count = self.pick_count(&kind);
                let select_mixed_dungeons = !self.settings.contains_key("mixed-dungeons")
                    && self
                        .settings
                        .get("dungeon-er")
                ...
                let team = match (kind, pick_count, went_first) {
                    ...
                    | (_, 6, true)
                    | (_, 7, false)
                    | (Kind::TournoiFrancoS3, 8, true)
                    | (Kind::TournoiFrancoS3, 9, false) => Team::HighSeed,
                    (_, 0, false)
                    | (_, 1, true)
                    | (_, 2, false)
                    | (_, 3, true)
                    ...
                    | (_, 6, false)
                    | (_, 7, true)
                    | (Kind::TournoiFrancoS3, 8, false)
                    | (Kind::TournoiFrancoS3, 9, true) => Team::LowSeed,
                    (Kind::PickOnly { .. }, 8.., _)
                    | (Kind::BanPick { .. }, 8.., _)
                    | (Kind::BanOnly { .. }, 8.., _) => unreachable!(),
                    (Kind::TournoiFrancoS3, 10.., _)
                    | (Kind::TournoiFrancoS4 | Kind::TournoiFrancoS5, 8.., _) => {
                        return Ok(Step {
                            kind: StepKind::Done(match kind {
                                Kind::TournoiFrancoS3 => {
                                    fr::resolve_s3_draft_settings(&self.settings)
                                }
                                Kind::TournoiFrancoS4 => {
                                    fr::resolve_s4_draft_settings(&self.settings)
                                }
                                Kind::TournoiFrancoS5 => {
                                    fr::resolve_s5_draft_settings(&self.settings)
                                }
                                Kind::MultiworldS3
                                | Kind::MultiworldS4
                                | Kind::MultiworldS5
                            ...
                            message: match msg_ctx {
                                MessageContext::None => String::default(),
                                MessageContext::Discord { .. } => {
                                    if let French = kind.language() {
                                        format!(
                                            "Fin du draft ! Voici un récapitulatif : {}.",
                                            fr::display_draft_picks(
                                                kind.language(),
                                                all_settings,
                                                &self.settings
                                            )
                                ...
                                MessageContext::RaceTime { .. } => fr::display_draft_picks(
                                    kind.language(),
                                    all_settings,
                                    &self.settings,
                                ),
                ...
                if select_mixed_dungeons {
                    ...
                    match self.pick_count(&kind) {
                        prev_bans @ 0..=1 => {
                            let hard_settings_ok = self
                                .settings
                                .get("hard_settings_ok")
                                .map(|hard_settings_ok| &**hard_settings_ok)
                                .unwrap_or("no")
                                == "ok";
                            let (hard_settings, classic_settings) = all_settings
                                .iter()
                                .filter(|&&fr::Setting { name, .. }| {
                                    !self.settings.contains_key(name)
                                        && match name {
                                            "keysy" => self
                                                .settings
                                                .get("keysanity")
                                                .is_none_or(|keysanity| keysanity == "off"),
                                            "1major" if *kind == Kind::TournoiFrancoS5 => {
                                                self.settings.get("th").is_none_or(|th| th == "off")
                                                    && self
                                                        .settings
                                                        .get("souls")
                                                        .is_none_or(|souls| souls == "off")
                                            }
                                            "souls" if *kind == Kind::TournoiFrancoS5 => self
                                                .settings
                                                .get("1major")
                                                .is_none_or(|one_major| one_major == "off"),
                                            "th" if *kind == Kind::TournoiFrancoS5 => self
                                                .settings
                                                .get("1major")
                                                .is_none_or(|one_major| one_major == "off"),
                                            "keysanity" => self
                                    ...
                                    |&fr::Setting {
                                        ...
                                        (!is_empty).then(|| {
                                            (
                                                is_hard,
                                                BanSetting {
                                                    display: (if display == "enemy souls"
                                                        && !hard_settings_ok
                                                    {
                                                        "boss souls"
                                                    } else {
                            ...
                            let mut available_settings = vec![(
                                Cow::Borrowed(if let French = kind.language() {
                                    "Settings classiques"
                                } else {
                                    "Classic Settings"
                                }),
                            ...
                            Step {
                                ...
                                message: match msg_ctx {
                                    ...
                                    MessageContext::Discord {
                                        ...
                                        let (mut high_seed, mut low_seed) =
                                            teams.iter().partition::<Vec<_>, _>(|team| {
                                                team.id == self.high_seed
                                            });
                                        let high_seed = high_seed.remove(0);
                                        let low_seed = low_seed.remove(0);
                                        MessageBuilder::default()
                                    ...
                                    MessageContext::RaceTime {
                                        ...
                                        if let French = kind.language() {
                                            format!(
                                                ...
                                                team.choose(high_seed_name, low_seed_name),
                                                if prev_bans == 0 {
                                                    " Utilisez “!settings” pour la liste des settings."
                                                } else {
                                                    ""
                                                },
                                            )
                                ...
                                Kind::TournoiFrancoS3 | Kind::TournoiFrancoS4 => {
                                    n < round_count - 2
                                        || self
                                            .settings
                                            .get(team.choose(
                                                "high_seed_has_picked",
                                                "low_seed_has_picked",
                                            ))
                                            .map(|has_picked| &**has_picked)
                                            .unwrap_or("no")
                                            == "yes"
                                }
                                        ...
                                        (!other.is_empty()).then(|| {
                                            (
                                                ...
                                                DraftSetting {
                                                    ...
                                                    options: can_ban
                                                        .then(|| DraftSettingChoice {
                                                            name: default,
                                                            display: default_display.into(),
                                                        })
                                                        .into_iter()
                                                        .chain(other.into_iter().map(
                                                            |(name, _, display)| {
                                                                DraftSettingChoice {
                                                                    name,
                                                                    display: display.into(),
                            ...
                            Step {
                                ...
                                message: match msg_ctx {
                                        ...
                                        match (kind, n) {
                                            (_, 9) | (Kind::TournoiFrancoS4, 7) => {
                                                if let French = kind.language() {
                                                    ...
                                                    builder.mention_command(
                                                        command_ids.pick.unwrap(),
                                                        "pick",
                                                    );
                                                    builder.push('.');
                                                    if skippable {
                                    ...
                                    MessageContext::RaceTime {
                                        ...
                                        (Kind::TournoiFrancoS4, 7) | (_, 9) if skippable => {
                                            if let French = kind.language() {
                                                format!(
                                                    "{}, choisissez le dernier setting. Vous pouvez également utiliser “!skip” si vous voulez laisser les settings comme ils sont.",
                                                    team.choose(high_seed_name, low_seed_name)
                                                )
                                            } else {
                                                format!(
                                        ...
                                        (Kind::TournoiFrancoS4, 7) | (_, 9) => {
                                            if let French = kind.language() {
                                                format!(
                                                    "{}, choisissez votre dernier setting.",
                                                    team.choose(high_seed_name, low_seed_name)
                                                )
                                            } else {
                                                format!(
                                                    "{}, pick the final setting.",
                                                    team.choose(high_seed_name, low_seed_name)
                                                )
                                            }
                                        }
                                        (_, 2) => {
                                            if let French = kind.language() {
                                                format!(
                                                    "{}, choisissez un setting avec “!pick <setting> <configuration>”. <configuration> signifie la valeur du setting. Par exemple pour tokensanity, la configuration peut être {{all, dungeon, overworld}}.",
                                                    team.choose(high_seed_name, low_seed_name)
                                                )
                                            } else {
                                                format!(
                                        ...
                                        (_, 3 | 5) => {
                                            if let French = kind.language() {
                                                format!(
                                                    "{}, choisissez deux settings. Quel est votre premier ?",
                                                    team.choose(high_seed_name, low_seed_name)
                                                )
                                            } else {
                                                format!(
                ...
                Step {
                    ...
                    message: match msg_ctx {
                        ...
                        MessageContext::Discord {
                            ...
                            if let French = kind.language() {
                                ...
                                builder
                                    ...
                                    .await?;
                                builder.push(
                                    ": you have won the coin flip. Choose whether you want to go ",
                                );
                                builder.mention_command(command_ids.first.unwrap(), "first");
                                builder.push(" or ");
                                builder.mention_command(command_ids.second.unwrap(), "second");
                        ...
                        MessageContext::RaceTime { high_seed_name, .. } => {
                            if let French = kind.language() {
                                format!(
                                    "{high_seed_name}, vous avez été sélectionné pour décider qui commencera le draft en premier. Si vous voulez commencer, veuillez entrer “!first”. Autrement, entrez “!second”."
                                )
                            } else {
                                format!(
                                    "{high_seed_name}, you have won the coin flip. Choose whether you want to go !first or !second in the settings draft."
                                )
                            }
                        }
                    },
    ...
    async fn next_step_pick_only(
        ...
        if self.settings.contains_key(&*last_game_key) {
            ...
            let preset_val = self
                .settings
                .get(&*format!("game{game_num}_preset"))
                .expect("PickOnly draft complete but game preset missing");
            let preset_display = options
                .iter()
                .find(|p| p.preset == preset_val.as_ref())
                .map(|p| p.display_name.as_str())
                .unwrap_or(preset_val.as_ref());
            ...
            let picks_made = (1..=total_picks)
                .filter(|&n| self.settings.contains_key(&*format!("game{n}_preset")))
                .count();
            // Whose turn: who_starts picks on even counts, the other on odd
            let team = if picks_made % 2 == 0 {
                who_starts
            } else {
                match who_starts {
                    Team::HighSeed => Team::LowSeed,
                    Team::LowSeed => Team::HighSeed,
                }
            };
            ...
            let available_presets: Vec<PresetOption> = if unique {
                ...
                options
                    .iter()
                    .filter(|p| !taken.contains(&&*p.preset))
                    .cloned()
                    .collect()
            } else {
                options.to_vec()
            };
            let preset_list = available_presets
                .iter()
                .map(|p| &*p.display_name)
                .collect::<Vec<_>>()
                .join(", ");
            Ok(Step {
                kind: StepKind::PickPreset {
                    team,
                    available_presets,
                    game: next_game,
                },
                message: match msg_ctx {
                    ...
                    MessageContext::Discord {
                        ...
                        let (mut high, mut low): (Vec<_>, Vec<_>) =
                            teams.iter().partition(|t| t.id == self.high_seed);
                        let active = team.choose(high.remove(0), low.remove(0));
                        let mut builder = MessageBuilder::default();
                        if picks_made > 0 {
                            let prev_preset = self
                                .settings
                                .get(&*format!("game{picks_made}_preset"))
                                .map(|s| s.as_ref())
                                .unwrap_or("?");
                            let prev_display = options
                                .iter()
                                .find(|p| p.preset == prev_preset)
                                .map(|p| p.display_name.as_str())
                                .unwrap_or(prev_preset);
                            builder.push(format!(
                                "**{prev_display}** was picked for Game {picks_made}.\n"
                            ));
                        }
                        builder
                            .mention_team(transaction, Some(*guild_id), active)
                            .await?;
                    ...
                    MessageContext::RaceTime {
                        ...
                        format!(
                            "{name}, pick the {label} for Game {next_game}. Available: {preset_list}"
                        )
                    }
                },
            })
    ...
    async fn next_step_ban_pick(
        ...
        let decider_game = order
            .iter()
            .filter(|p| matches!(p, DraftPhase::Pick(_)))
            .count()
            + 1;
        let decider_key = format!("game{decider_game}_preset");
        if self.settings.contains_key(decider_key.as_str()) {
            // The final unselected preset is assigned after all configured picks.
            let game_num = game.unwrap_or(1) as usize;
            let preset_val = self
                .settings
                .get(&*format!("game{game_num}_preset"))
                ...
                message: match msg_ctx {
                    ...
                    MessageContext::Discord { .. } => {
                        if game.unwrap_or(1) == 1 {
                            ...
                            if let Some(v) = self.settings.get(decider_key.as_str()) {
                                let d = options
                                    .iter()
                                    .find(|p| p.preset == v.as_ref())
                                    .map(|p| p.display_name.as_str())
                                    .unwrap_or(v.as_ref());
                                parts.push(format!("Game {decider_game}: {d}"));
                            }
                            parts.join("\n")
                        } else {
                            String::default()
                ...
                match phase {
                    DraftPhase::Ban(team) => {
                        ...
                        if !self.settings.contains_key(&*format!("ban{ban_count}")) {
                            ...
                            let banned: Vec<&str> = (1..ban_count)
                                .filter_map(|n| {
                                    self.settings.get(&*format!("ban{n}")).map(|s| s.as_ref())
                                })
                                .collect();
                            let picked: Vec<&str> = (1..=pick_count)
                                .filter_map(|n| {
                                    self.settings
                                        .get(&*format!("game{n}_preset"))
                                        .map(|s| s.as_ref())
                                ...
                                .collect();
                            let available: Vec<BanSetting> = options
                                .iter()
                                .filter(|p| {
                                    !banned.contains(&p.preset.as_str())
                                        && !picked.contains(&p.preset.as_str())
                                })
                                .map(|p| BanSetting {
                                    name: p.preset.clone().into(),
                                    display: p.display_name.clone().into(),
                                    default: p.preset.clone().into(),
                                    default_display: p.display_name.clone().into(),
                                    description: Cow::Owned(p.display_name.clone()),
                                })
                                .collect();
                            let avail_display = available
                                .iter()
                                .map(|s| s.display.as_ref())
                                .collect::<Vec<_>>()
                                .join(", ");
                            return Ok(Step {
                                kind: StepKind::Ban {
                                ...
                                message: match msg_ctx {
                                    ...
                                    MessageContext::RaceTime {
                                        ...
                                        let name = team.choose(*high_seed_name, *low_seed_name);
                                        format!(
                                            "{name}, ban a {label} using \"!ban <{label}>\". Available: {avail_display}"
                                        )
                                    }
                                },
                            });
                    ...
                    DraftPhase::Pick(team) => {
                        ...
                        if !self.settings.contains_key(&*format!("pick{pick_count}")) {
                            ...
                            let picked: Vec<&str> = (1..pick_count)
                                ...
                                .collect();
                            let available_presets: Vec<PresetOption> = options
                                .iter()
                                .filter(|p| {
                                    !banned.contains(&&*p.preset) && !picked.contains(&&*p.preset)
                                })
                                .cloned()
                                .collect();
                            let avail_display = available_presets
                                .iter()
                                .map(|p| &*p.display_name)
                                .collect::<Vec<_>>()
                                .join(", ");
                                ...
                                message: match msg_ctx {
                                    ...
                                    MessageContext::Discord {
                                        ...
                                        builder
                                            ...
                                            .await?;
                                        builder.push(format!(
                                            ": Pick the {label} for Game {current_pick}."
                                        ));
                                        builder.build()
                                    }
                                    MessageContext::RaceTime {
                                        ...
                                        let name = team.choose(*high_seed_name, *low_seed_name);
                                        format!(
                                            "{name}, pick the {label} for Game {current_pick}. Available: {avail_display}"
                                        )
                                    }
                                },
                            });
                        ...
                        let val = self
                            .settings
                            .get(&*format!("game{pick_count}_preset"))
                            .map(|s| s.as_ref())
                            .unwrap_or("?");
                        let display = options
                            .iter()
                            ...
                            .unwrap_or(val);
                        prev_action =
                            Some(format!("**{display}** was picked for Game {pick_count}."));
                    }
                }
            }
    ...
    async fn next_step_ban_only(
        ...
        let game_count = options.len().saturating_sub(order.len());
        if self
            .settings
            .contains_key(format!("game{game_count}_preset").as_str())
        {
                ...
                match phase {
                    DraftPhase::Ban(team) => {
                        ...
                        if !self.settings.contains_key(&*format!("ban{ban_count}")) {
                            ...
                            let available: Vec<BanSetting> = options
                                .iter()
                                .filter(|p| !banned.contains(&p.preset.as_str()))
                                .map(|p| BanSetting {
                                    name: p.preset.clone().into(),
                                    display: p.display_name.clone().into(),
                                    default: p.preset.clone().into(),
                    ...
                    DraftPhase::Pick(_) => {
                        unreachable!("BanOnly order should not contain Pick phases")
                    }
                }
            }
    ...
    pub(crate) async fn next_step(
        ...
        match kind {
            ...
            Kind::BanPick {
                ...
                self.next_step_ban_pick(options, order, label, game, msg_ctx)
                    .await
            }
            Kind::BanOnly {
                options,
                ...
                self.next_step_ban_only(options, order, label, game, msg_ctx)
                    .await
            }
        }
    }
    pub(crate) async fn active_team(
        ...
        Ok(
            match self
                .next_step(kind, game, &mut MessageContext::None)
                .await?
                .kind
            {
                ...
                StepKind::Ban { team, .. }
                | StepKind::Pick { team, .. }
                | StepKind::BooleanChoice { team }
                | StepKind::PickPreset { team, .. } => Some(team),
    ...
    pub(crate) async fn is_active_team(
        &self,
        kind: &Kind,
        game: Option<i16>,
        team: Id<Teams>,
    ...
    async fn apply_s7(
        ...
        Ok({
            let resolved_action = match action {
                Action::Ban { setting } => {
                    if let Some(setting) = s::S7_SETTINGS
                        .into_iter()
                        .find(|&s::Setting { name, .. }| *name == setting)
                    {
                        Action::Pick {
                ...
                Action::BooleanChoice(value)
                    if matches!(
                        self.next_step_s7(game, &mut MessageContext::None)
                            .await?
                            .kind,
                        StepKind::GoFirst
                    ) =>
                {
                    Action::GoFirst(value)
                }
                _ => action,
            };
            match resolved_action {
                Action::GoFirst(first) => match self
                    .next_step_s7(game, &mut MessageContext::None)
                    .await?
                    .kind
                {
                    StepKind::GoFirst => {
                        ...
                        Ok(match msg_ctx {
                            MessageContext::None | MessageContext::RaceTime { .. } => {
                                String::default()
                            }
                            MessageContext::Discord {
                                transaction,
                                ...
                                .mention_team(transaction, Some(*guild_id), team)
                                .await?
                                .push(if team.name_is_plural() {
                                    " have"
                                } else {
                                    " has"
                                })
                ...
                Action::Pick { setting, value } => match self
                    .next_step_s7(game, &mut MessageContext::None)
                    .await?
                    .kind
                {
                    StepKind::GoFirst => Err(match msg_ctx {
                        ...
                        MessageContext::Discord { command_ids, .. } => MessageBuilder::default()
                            ...
                            .build(),
                        MessageContext::RaceTime { reply_to, .. } => format!(
                            "Sorry {reply_to}, first pick hasn't been chosen yet, use “!first” or “!second”"
                        ),
                    }),
                    StepKind::Ban {
                        available_settings,
                        ...
                        if let Some(setting) = available_settings.get(&setting) {
                            if value == setting.default {
                                self.settings
                                    .insert(setting.name.clone(), setting.default.clone());
                                Ok(match msg_ctx {
                                    MessageContext::None | MessageContext::RaceTime { .. } => {
                                        String::default()
                                    }
                                ...
                                Err(match msg_ctx {
                                    ...
                                    MessageContext::RaceTime { reply_to, .. } => format!(
                                        "Sorry {reply_to}, bans haven't been chosen yet. Use “!ban <setting>”"
                                    ),
                                })
                            }
                        } else {
                            let exists = s::S7_SETTINGS
                                .into_iter()
                                .any(|s::Setting { name, .. }| setting == name);
                            Err(match msg_ctx {
                                MessageContext::None => String::default(),
                                MessageContext::Discord { command_ids, .. } => {
                                    let mut content = MessageBuilder::default();
                                ...
                                MessageContext::RaceTime { reply_to, .. } => format!(
                                    ...
                                    if exists {
                                        "that setting is already locked in"
                                    } else {
                                        "I don't recognize that setting"
                                    },
                                    available_settings
                                        .all()
                                        .map(|setting| setting.name)
                                        .format(" or "),
                                    if exists && skippable {
                                        ". Use “!skip” if you don't want to ban anything."
                                    } else {
                                        ""
                                    },
                                ),
                    ...
                    StepKind::Pick {
                        ...
                        if let Some(setting) = available_choices.get(&setting) {
                            if let Some(option) =
                                setting.options.iter().find(|option| option.name == value)
                            {
                            ...
                            Err(match msg_ctx {
                                ...
                                MessageContext::RaceTime { reply_to, .. } => format!(
                                    ...
                                    available_choices
                                        ...
                                        .format(" or "),
                                    if exists && skippable {
                                        ". Use “!skip” if you don't want to pick anything."
                                    } else {
                                        ""
                                    },
                                ),
                    ...
                    .kind
                {
                    ...
                    | StepKind::Pick {
                        ...
                        Ok(match msg_ctx {
                            ...
                            MessageContext::Discord {
                                ...
                                .mention_team(&mut *transaction, Some(*guild_id), team)
                                .await?
                                .push(if team.name_is_plural() {
                                    " have skipped "
                                } else {
                    ...
                    | StepKind::Pick {
                        ...
                        MessageContext::None => String::default(),
                        MessageContext::Discord { .. } => {
                            format!("Sorry, this part of the draft can't be skipped.")
                        }
                        MessageContext::RaceTime { reply_to, .. } => {
                            format!("Sorry {reply_to}, this part of the draft can't be skipped.")
                        }
                    }),
                    StepKind::BooleanChoice { .. }
                    | StepKind::DoneRsl { .. }
                ...
                Action::BooleanChoice(_) => match self
                    .next_step_s7(game, &mut MessageContext::None)
                    .await?
                    .kind
                {
    ...
    async fn apply_multiworld_s3(
        ...
        Ok({
            let resolved_action = match action {
                Action::Ban { setting } => {
                    if let Some(setting) = mw::S3_SETTINGS
                        .iter()
                        .copied()
                        .find(|&mw::Setting { name, .. }| *name == setting)
                    {
                        Action::Pick {
                            setting: setting.name.to_owned(),
                ...
                Action::BooleanChoice(value)
                    if matches!(
                        self.next_step_multiworld_s3(game, &mut MessageContext::None)
                            .await?
                            .kind,
                        StepKind::GoFirst
                    ) =>
            ...
            match resolved_action {
                Action::GoFirst(first) => match self
                    .next_step_multiworld_s3(game, &mut MessageContext::None)
                    .await?
                    .kind
                {
                    StepKind::GoFirst => {
                    ...
                    .kind
                {
                    ...
                    StepKind::Ban {
                        ...
                        if let Some(setting) = available_settings.get(&setting) {
                            ...
                            let exists = mw::S3_SETTINGS
                                .iter()
                                .copied()
                                .any(|mw::Setting { name, .. }| setting == name);
                            Err(match msg_ctx {
                                MessageContext::None => String::default(),
                                MessageContext::Discord { command_ids, .. } => {
                                    let mut content = MessageBuilder::default();
    ...
    async fn apply_multiworld_s4(
        ...
        Ok({
            let resolved_action = match action {
                Action::Ban { setting } => {
                    if let Some(setting) = mw::S4_SETTINGS
                        .iter()
                        .copied()
                        .find(|&mw::Setting { name, .. }| *name == setting)
                    {
                ...
                Action::BooleanChoice(value)
                    if matches!(
                        self.next_step_multiworld_s4(game, &mut MessageContext::None)
                            .await?
                            .kind,
                        StepKind::GoFirst
                    ) =>
            ...
            match resolved_action {
                Action::GoFirst(first) => match self
                    .next_step_multiworld_s4(game, &mut MessageContext::None)
                    .await?
                    .kind
                {
                    StepKind::GoFirst => {
    ...
    async fn apply_multiworld_s5(
        ...
        Ok({
            let resolved_action = match action {
                Action::Ban { setting } => {
                    if let Some(setting) = mw::S5_SETTINGS
                        .iter()
                        .copied()
                        .find(|&mw::Setting { name, .. }| *name == setting)
                    {
                ...
                Action::BooleanChoice(value)
                    if matches!(
                        self.next_step_multiworld_s5(game, &mut MessageContext::None)
                            .await?
                            .kind,
                        StepKind::GoFirst
                    ) =>
            ...
            match resolved_action {
                Action::GoFirst(first) => match self
                    .next_step_multiworld_s5(game, &mut MessageContext::None)
                    .await?
                    .kind
                {
                    StepKind::GoFirst => {
    ...
    async fn apply_rsl_s7(
        ...
        Ok({
            let resolved_action = match action {
                ...
                Action::BooleanChoice(value)
                    if matches!(
                        self.next_step_rsl_s7(game, &mut MessageContext::None)
                            .await?
                            .kind,
                        StepKind::GoFirst
                    ) =>
            ...
            match resolved_action {
                Action::GoFirst(first) => match self
                    .next_step_rsl_s7(game, &mut MessageContext::None)
                    .await?
                    .kind
                {
                    StepKind::GoFirst => {
                        ...
                        Ok(match msg_ctx {
                            ...
                            MessageContext::Discord {
                                ...
                                content
                                    .mention_team(transaction, Some(*guild_id), team)
                                    .await?;
                                content.push(if team.name_is_plural() {
                                    " have"
                                } else {
                                    " has"
                                });
                    ...
                    StepKind::DoneRsl { .. } => Err(match msg_ctx {
                        MessageContext::None => String::default(),
                        MessageContext::Discord { .. } => {
                            format!("Sorry, this weights draft is already completed.")
                        }
                        MessageContext::RaceTime { reply_to, .. } => {
                            format!("Sorry {reply_to}, this weights draft is already completed.")
                        }
                    }),
                },
                Action::Ban { .. } => unreachable!("normalized to Action::Pick above"),
                            ...
                            Err(match msg_ctx {
                                ...
                                MessageContext::RaceTime { reply_to, .. } => format!(
                                    ...
                                    available_settings
                                        ...
                                        .format(" or "),
                                    if skippable {
                                        ". Use “!skip” if you don't want to block anything."
                                    } else {
                                        ""
                                    },
                                ),
                                ...
                                setting.options.iter().find(|option| option.name == value)
                            {
                                match self.settings.entry(Cow::Borrowed(setting.name)) {
                                    ...
                                    hash_map::Entry::Vacant(entry) => {
                                        entry.insert(Cow::Borrowed(option.name));
                                    }
                                }
                                match self
                                    .settings
                                    .entry(Cow::Owned(format!("{}_banned_by", setting.name)))
                                {
                                    hash_map::Entry::Occupied(mut entry) => {
                                        let entry = entry.get_mut();
                                    ...
                                    hash_map::Entry::Vacant(entry) => {
                                        entry.insert(Cow::Owned(team.to_string()));
                                    }
                                }
                                Ok(match msg_ctx {
                                    MessageContext::None | MessageContext::RaceTime { .. } => {
    ...
    async fn apply_tournoi_franco(
        ...
        Ok({
            ...
            let resolved_action = match action {
                Action::Ban { setting } => {
                    if let Some(setting) = all_settings
                        .iter()
                        .find(|&&fr::Setting { name, .. }| *name == setting)
                    {
                        Action::Pick {
                            setting: setting.name.to_owned(),
                            value: setting.default.to_owned(),
            ...
            match resolved_action {
                Action::GoFirst(first) => match self
                    .next_step_tournoi_franco(kind, game, &mut MessageContext::None)
                    .await?
                    .kind
                {
                    StepKind::GoFirst => {
                            ...
                            MessageContext::Discord {
                                ...
                                if let French = kind.language() {
                                    ...
                                    if self
                                        ...
                                        == "ok"
                                    {
                                        let mq_dungeons_count = self
                                            .settings
                                            .get("mq_dungeons_count")
                                            .map(|mq_dungeons_count| &**mq_dungeons_count)
                                            .unwrap_or("0");
                    ...
                    StepKind::Ban { .. } | StepKind::Pick { .. } => Err(match msg_ctx {
                        ...
                        MessageContext::RaceTime { reply_to, .. } => {
                            if let French = kind.language() {
                                format!(
                                    "Désolé {reply_to}, le premier pick a déjà été sélectionné."
                                )
                            } else {
                                format!("Sorry {reply_to}, first pick has already been chosen.")
                            }
                    ...
                    StepKind::BooleanChoice { .. } => Err(match msg_ctx {
                        ...
                        MessageContext::RaceTime { reply_to, .. } => {
                            if let French = kind.language() {
                                format!(
                                    "Désolé {reply_to}, avant que le draft ne puisse continuer, vous devez d'abord choisir si les donjons seront mixés ou non avec le reste. Utilisez !yes ou !no"
                                )
                            } else {
                                format!(
                                    "Sorry {reply_to}, before the settings draft can continue, you first have to choose whether dungeons entrances should be mixed. Use !yes or !no"
                                )
                            }
                        }
                    }),
                    ...
                    StepKind::Done(_) => Err(match msg_ctx {
                        ...
                        MessageContext::RaceTime { reply_to, .. } => {
                            if let French = kind.language() {
                                ...
                                format!(
                                    "Sorry {reply_to}, this settings draft is already completed."
                                )
                            }
                        }
                    }),
                    ...
                    .kind
                {
                    ...
                    StepKind::Ban {
                        ...
                        if let Some(setting) = available_settings.get(&setting) {
                                    ...
                                    MessageContext::Discord {
                                        ...
                                        if let French = kind.language() {
                                            MessageBuilder::default()
                                                ...
                                                .await?
                                                .push(" a banni ")
                                                .push(match &*setting.name {
                                                    "camc" => "no CAMC",
                                                    "souls" if !hard_settings_ok => "boss souls",
                                                    _ => &*setting.display,
                                                })
                                                .push('.')
                                                .build()
                            ...
                            let exists = all_settings
                                .iter()
                                .any(|&fr::Setting { name, .. }| setting == name);
                            Err(match msg_ctx {
                                MessageContext::None => String::default(),
                                MessageContext::Discord { command_ids, .. } => {
                                    let mut content = MessageBuilder::default();
                        ...
                        if let Some(setting) = available_choices.get(&setting) {
                                ...
                                setting.options.iter().find(|option| option.name == value)
                            {
                                ...
                                let is_default = value
                                    == all_settings
                                        .iter()
                                        .find(|&&fr::Setting { name, .. }| setting.name == name)
                                        .unwrap()
                                        .default;
                                if !is_default {
                                    self.settings.insert(
                                        Cow::Borrowed(
                                            self.active_team(kind, game).await?.unwrap().choose(
                                                "high_seed_has_picked",
                                                "low_seed_has_picked",
                                            ),
                                        ),
                ...
                Action::BooleanChoice(value) => match self
                    .next_step_tournoi_franco(kind, game, &mut MessageContext::None)
                    .await?
                    .kind
                {
                    StepKind::BooleanChoice { .. } => {
                        self.settings.insert(
                            Cow::Borrowed("mixed-dungeons"),
                            Cow::Borrowed(if value { "mixed" } else { "separate" }),
                        );
                        Ok(match msg_ctx {
                            MessageContext::None | MessageContext::RaceTime { .. } => {
                                String::default()
                    ...
                    _ => Err(match msg_ctx {
                        ...
                        MessageContext::RaceTime { reply_to, .. } => {
                            if let French = kind.language() {
                                ...
                                format!(
                                    "Sorry {reply_to}, the current step is not a yes/no question."
                                )
                            }
                        }
                    }),
    ...
    async fn apply_pick_only(
        ...
        Ok(match step.kind {
            StepKind::Done(_) => Err(match msg_ctx {
                MessageContext::None => String::default(),
                MessageContext::Discord { .. } => {
                    format!("Sorry, the preset draft is already completed.")
                }
                MessageContext::RaceTime { reply_to, .. } => {
                    format!("Sorry {reply_to}, the preset draft is already completed.")
                }
            }),
            StepKind::PickPreset {
                team: _team,
                ...
                let preset_name = match action {
                    ...
                    _ => {
                        return Ok(Err(match msg_ctx {
                            MessageContext::None => String::default(),
                            MessageContext::Discord { .. } => {
                                format!("Please use /pick to choose a preset.")
                            }
                            MessageContext::RaceTime { reply_to, .. } => format!(
                                "Sorry {reply_to}, please use \"!pick <preset>\" to pick a preset."
                            ),
                        }));
                    }
                };
                ...
                let Some(preset_opt) = preset_opt else {
                    ...
                    return Ok(Err(match msg_ctx {
                        MessageContext::None => String::default(),
                        MessageContext::Discord { .. } => {
                            format!("Sorry, that preset is not available. Use one of: {avail}")
                        }
                        MessageContext::RaceTime { reply_to, .. } => format!(
                            "Sorry {reply_to}, that preset is not available. Use one of: {avail}"
                        ),
                    }));
                };
                let key = format!("game{next_game}_preset");
                self.settings
                    .insert(Cow::Owned(key), Cow::Owned(preset_opt.preset.clone()));
                Ok(match msg_ctx {
                    ...
                    MessageContext::Discord {
                        ...
                        .mention_team(transaction, Some(*guild_id), team_data)
                        .await?
                        .push(if team_data.name_is_plural() {
                            " have picked "
                        } else {
                            " has picked "
                        })
    ...
    async fn apply_ban_pick(
        ...
        let step = self
            .next_step_ban_pick(options, order, label, game, &mut MessageContext::None)
            .await?;
        Ok(match step.kind {
            StepKind::Done(_) => Err(match msg_ctx {
                MessageContext::None => String::default(),
                ...
                let preset_name = match action {
                    ...
                    _ => {
                        return Ok(Err(match msg_ctx {
                            ...
                            MessageContext::RaceTime { reply_to, .. } => format!(
                                "Sorry {reply_to}, please use \"!ban <preset>\" to ban a preset."
                            ),
                        }));
                    }
                };
                if available_settings.get(&preset_name).is_none() {
                    let avail = available_settings
                        .all()
                    ...
                    return Ok(Err(match msg_ctx {
                        MessageContext::None => String::default(),
                        MessageContext::Discord { .. } => format!(
                            "Sorry, that preset is not available to ban. Available: {avail}"
                        ),
                        MessageContext::RaceTime { reply_to, .. } => format!(
                            "Sorry {reply_to}, that preset is not available to ban. Available: {avail}"
                        ),
                    }));
                }
                // Store as ban{N}
                let ban_count = (1..)
                    .find(|n| !self.settings.contains_key(&*format!("ban{n}")))
                    .unwrap();
                let display = options
                    .iter()
                    .find(|p| p.preset == preset_name)
                    .map(|p| p.display_name.to_owned())
                    .unwrap_or_else(|| preset_name.clone());
                self.settings.insert(
                    Cow::Owned(format!("ban{ban_count}")),
                    Cow::Owned(preset_name),
                );
                Ok(match msg_ctx {
                    MessageContext::None | MessageContext::RaceTime { .. } => String::default(),
                    MessageContext::Discord {
            ...
            StepKind::PickPreset {
                ...
                let pick_num = (1..)
                    .find(|n| !self.settings.contains_key(&*format!("pick{n}")))
                    .unwrap();
                self.settings.insert(
                    Cow::Owned(format!("pick{pick_num}")),
                    Cow::Owned(preset_opt.preset.clone()),
                );
                self.settings.insert(
                    Cow::Owned(format!("game{current_pick}_preset")),
                    Cow::Owned(preset_opt.preset.clone()),
                );
                ...
                // Preserve existing gameN keys while supporting other match lengths.
                if pick_num == total_picks {
                    let ban_count = order
                        .iter()
                        .filter(|p| matches!(p, DraftPhase::Ban(_)))
                        .count();
                    let banned: Vec<&str> = (1..=ban_count)
                        .filter_map(|n| self.settings.get(&*format!("ban{n}")).map(|s| s.as_ref()))
                        .collect();
                    let picked: Vec<&str> = (1..=total_picks)
                        .filter_map(|n| {
                            self.settings
                                .get(&*format!("game{n}_preset"))
                                .map(|s| s.as_ref())
                    ...
                    let remaining = options
                        ...
                        .expect("validated draft should leave a decider preset");
                    self.settings.insert(
                        Cow::Owned(format!("game{}_preset", total_picks + 1)),
                        Cow::Owned(remaining.preset.clone()),
                    );
                }
                response
            }
    ...
    async fn apply_ban_only(
        ...
        let step = self
            .next_step_ban_only(options, order, label, game, &mut MessageContext::None)
            .await?;
        Ok(match step.kind {
            StepKind::Done(_) => Err(match msg_ctx {
                MessageContext::None => String::default(),
            ...
            StepKind::Ban {
                ...
                let total_bans = order
                    .iter()
                    .filter(|p| matches!(p, DraftPhase::Ban(_)))
                    .count();
                if ban_count == total_bans {
                    let banned: Vec<&str> = (1..=total_bans)
                        .filter_map(|n| self.settings.get(&*format!("ban{n}")).map(|s| s.as_ref()))
                        .collect();
                    let mut remaining: Vec<PresetOption> = options
                        .iter()
                        .filter(|p| !banned.contains(&p.preset.as_str()))
                        .cloned()
                        .collect();
                    ...
                    for (i, preset) in remaining.iter().enumerate() {
                        self.settings.insert(
                            Cow::Owned(format!("game{}_preset", i + 1)),
                            Cow::Owned(preset.preset.clone()),
                        );
                    }
                }
    ...
    pub(crate) async fn apply(
        ...
        match kind {
            ...
            Kind::BanPick {
                ...
                self.apply_ban_pick(options, order, label, game, msg_ctx, action)
                    .await
            }
            Kind::BanOnly {
                options,
                ...
                self.apply_ban_only(options, order, label, game, msg_ctx, action)
                    .await
            }
        }
    }
    pub(crate) async fn complete_randomly(mut self, kind: &Kind) -> Result<Picks, Error> {
        Ok(loop {
            let action = match self
                .next_step(kind, None, &mut MessageContext::None)
                .await?
                .kind
            {
                ...
                StepKind::Ban {
                    ...
                    if let Some(setting) = settings
                        .into_iter()
                        .choose(&mut rng())
                        .expect("no available settings")
                    {
                        Action::Ban {
                            setting: setting.name.clone().into_owned(),
                        }
                    } else {
                        Action::Skip
                    }
                ...
                StepKind::PickPreset {
                    ...
                    let preset = available_presets
                        .into_iter()
                        .choose(&mut rng())
                        .expect("no available presets for PickPreset step");
                    Action::Pick {
                        setting: preset.preset.to_owned(),
                        value: preset.preset.to_owned(),
                    }
                }
                StepKind::BooleanChoice { .. } => Action::BooleanChoice(rng().random()),
                StepKind::Done(_) | StepKind::DoneRsl { .. } => break self.settings,
            };
            self.apply(kind, None, &mut MessageContext::None, action)
                .await?
                .expect("random draft made illegal action");
        })
    }
}
