use {
    crate::{prelude::*, user::RaceTimePronouns},
    rocket::{
        http::{Cookie, CookieJar, SameSite},
        outcome::Outcome,
    },
    rocket_oauth2::{OAuth2, TokenResponse},
    rocket_util::Error,
...
pub(crate) enum UserFromRequestError {
    #[error(transparent)]
    OAuth(#[from] rocket_oauth2::Error),
    #[error(transparent)]
    ...
    #[error(transparent)]
    Time(#[from] rocket::time::error::ConversionRange),
    #[error(transparent)]
...
async fn handle_racetime_token_response(
    http_client: &reqwest::Client,
    cookies: &CookieJar<'_>,
    token: &TokenResponse<RaceTime>,
) -> Result<RaceTimeUser, UserFromRequestError> {
    let mut cookie =
        Cookie::build(("racetime_token", token.access_token().to_owned())).same_site(SameSite::Lax);
    if let Some(expires_in) = token.expires_in() {
        cookie = cookie.max_age(
            Duration::from_secs(u64::try_from(expires_in)?.saturating_sub(60)).try_into()?,
        );
    }
    cookies.add_private(cookie);
    if let Some(refresh_token) = token.refresh_token() {
        cookies.add_private(
            Cookie::build(("racetime_refresh_token", refresh_token.to_owned()))
                .same_site(SameSite::Lax)
                .permanent(),
        );
    }
    Ok(http_client
        .get(format!("https://{}/o/userinfo", racetime_host()))
...
async fn handle_discord_token_response(
    http_client: &reqwest::Client,
    cookies: &CookieJar<'_>,
    token: &TokenResponse<Discord>,
) -> Result<DiscordUser, UserFromRequestError> {
    let mut cookie =
        Cookie::build(("discord_token", token.access_token().to_owned())).same_site(SameSite::Lax);
    if let Some(expires_in) = token.expires_in() {
        cookie = cookie.max_age(
            Duration::from_secs(u64::try_from(expires_in)?.saturating_sub(60)).try_into()?,
        );
    ...
    if let Some(refresh_token) = token.refresh_token() {
        cookies.add_private(
            Cookie::build(("discord_refresh_token", refresh_token.to_owned()))
                .same_site(SameSite::Lax)
                .permanent(),
        );
    }
...
async fn handle_challonge_token_response(
    http_client: &reqwest::Client,
    token: &TokenResponse<Challonge>,
) -> Result<ChallongeUser, UserFromRequestError> {
    Ok(http_client
        ...
        .detailed_error_for_status()
        .await?
        .json_with_text_in_error::<ChallongeResponse<_>>()
        .await?
        .data)
}

async fn handle_startgg_token_response(
    http_client: &reqwest::Client,
    token: &TokenResponse<StartGG>,
) -> Result<startgg::ID, UserFromRequestError> {
    let startgg::current_user_query::ResponseData {
        current_user:
            Some(startgg::current_user_query::CurrentUserQueryCurrentUser { id: Some(id) }),
    } = startgg::query_uncached::<startgg::CurrentUserQuery>(
        http_client,
        token.access_token(),
        startgg::current_user_query::Variables,
    )
    .await?
    else {
...
impl TryFrom<SerdeDiscriminator> for Discriminator {
    ...
    fn try_from(value: SerdeDiscriminator) -> Result<Self, InvalidDiscriminator> {
        let number = match value {
            ...
            SerdeDiscriminator::String(s) => {
                if regex_is_match!("^[0-9]{4}$", &s) {
                    s.parse()?
                } else {
        ...
        if number > 9999 {
            return Err(InvalidDiscriminator::Range(number));
        }
        Ok(Self(number))
    }
}
...
fn discord_opt_discriminator<'de, D: Deserializer<'de>>(
    deserializer: D,
) -> Result<Option<Discriminator>, D::Error> {
    ...
    async fn from_request(req: &'r Request<'_>) -> request::Outcome<Self, UserFromRequestError> {
        match req.guard::<&CookieJar<'_>>().await {
            Outcome::Success(cookies) => match req.guard::<&State<reqwest::Client>>().await {
                Outcome::Success(http_client) => {
                    if let Some(token) = cookies.get_private("racetime_token") {
                        match http_client
                            ...
                            .await
                        {
                            Ok(response) => Outcome::Success(guard_try!(
                        ...
                        match req.guard::<OAuth2<RaceTime>>().await {
                            Outcome::Success(oauth) => Outcome::Success(guard_try!(
                                handle_racetime_token_response(
                                    http_client,
                                    cookies,
                                    &guard_try!(oauth.refresh(token.value()).await)
                                )
                                .await
                            )),
                            Outcome::Error((status, ())) => {
                                Outcome::Error((status, UserFromRequestError::Cookie))
                            }
                ...
                Outcome::Error((status, ())) => {
                    Outcome::Error((status, UserFromRequestError::HttpClient))
                }
                Outcome::Forward(status) => Outcome::Forward(status),
            },
...
impl<'r> FromRequest<'r> for User {
    ...
    async fn from_request(req: &'r Request<'_>) -> request::Outcome<Self, UserFromRequestError> {
        // Route tests inject an identity through a request fairing; release builds
        // always use the real OAuth guards below.
        #[cfg(test)]
        if let Some(user) = req.local_cache(|| None::<User>).clone() {
        ...
        match req.guard::<&State<PgPool>>().await {
            Outcome::Success(pool) => {
                ...
                match req.guard::<RaceTimeUser>().await {
                    Outcome::Success(racetime_user) => {
                        if let Some(user) =
                            guard_try!(User::from_racetime(&**pool, &racetime_user.id).await)
                        {
                ...
                match req.guard::<DiscordUser>().await {
                    Outcome::Success(discord_user) => {
                        if let Some(user) =
                            guard_try!(User::from_discord(&**pool, discord_user.id).await)
                        {
            ...
            Outcome::Error((status, ())) => {
                Outcome::Error((status, UserFromRequestError::Database))
            }
            Outcome::Forward(status) => Outcome::Forward(status),
        }
    }
...
pub(crate) fn racetime_login(
    oauth: OAuth2<RaceTime>,
    cookies: &CookieJar<'_>,
    redirect_to: Option<Origin<'_>>,
) -> Result<Redirect, Error<rocket_oauth2::Error>> {
    if let Some(redirect_to) = redirect_to {
        if redirect_to.0.path() != uri!(racetime_callback).path()
            && redirect_to.0.path() != uri!(discord_callback).path()
        {
            // prevent showing login error page on login success
...
pub(crate) fn challonge_login(
    me: User,
    oauth: OAuth2<Challonge>,
    cookies: &CookieJar<'_>,
    redirect_to: Option<Origin<'_>>,
) -> Result<Redirect, Error<rocket_oauth2::Error>> {
...
pub(crate) fn startgg_login(
    oauth: OAuth2<StartGG>,
    cookies: &CookieJar<'_>,
    redirect_to: Option<Origin<'_>>,
) -> Result<Redirect, Error<rocket_oauth2::Error>> {
    if let Some(redirect_to) = redirect_to {
    ...
    oauth
        .get_redirect(cookies, &["user.identity"])
        .map_err(Error)
}
...
pub(crate) enum RaceTimeCallbackError {
    ...
    Sql(#[from] sqlx::Error),
    #[error(transparent)]
    UserFromRequest(#[from] UserFromRequestError),
}

#[rocket::get("/auth/racetime")]
pub(crate) async fn racetime_callback(
    pool: &State<PgPool>,
    me: Option<User>,
    http_client: &State<reqwest::Client>,
    token: TokenResponse<RaceTime>,
    cookies: &CookieJar<'_>,
) -> Result<Redirect, RaceTimeCallbackError> {
    let mut transaction = pool.begin().await?;
    let racetime_user = handle_racetime_token_response(http_client, cookies, &token).await?;
    let redirect_uri = cookies
        .get("redirect_to")
        .and_then(|cookie| rocket::http::uri::Origin::try_from(cookie.value()).ok())
        .map_or_else(|| uri!(crate::http::index), |uri| uri.into_owned());
    Ok(
        if User::from_racetime(&mut *transaction, &racetime_user.id)
            .await?
            .is_some()
        {
...
pub(crate) async fn discord_callback(
    ...
    token: TokenResponse<Discord>,
    cookies: &CookieJar<'_>,
) -> Result<Redirect, DiscordCallbackError> {
    let mut transaction = pool.begin().await?;
    ...
    let redirect_uri = cookies
        ...
        .map_or_else(|| uri!(crate::http::index), |uri| uri.into_owned());
    Ok(
        if User::from_discord(&mut *transaction, discord_user.id)
            .await?
            .is_some()
        {
            Redirect::to(redirect_uri)
...
pub(crate) async fn challonge_callback(
    pool: &State<PgPool>,
    me: User,
    http_client: &State<reqwest::Client>,
    token: TokenResponse<Challonge>,
    cookies: &CookieJar<'_>,
) -> Result<Redirect, ChallongeCallbackError> {
    let mut transaction = pool.begin().await?;
    let challonge_user = handle_challonge_token_response(http_client, &token).await?;
    sqlx::query!(
        "UPDATE users SET challonge_id = $1 WHERE id = $2",
        challonge_user.attributes.username,
        me.id as _
    )
...
pub(crate) async fn startgg_callback(
    pool: &State<PgPool>,
    me: User,
    http_client: &State<reqwest::Client>,
    token: TokenResponse<StartGG>,
    cookies: &CookieJar<'_>,
) -> Result<Redirect, StartGGCallbackError> {
    let mut transaction = pool.begin().await?;
    let startgg_id = handle_startgg_token_response(http_client, &token).await?;
    sqlx::query!(
        "UPDATE users SET startgg_id = $1 WHERE id = $2",
        startgg_id as _,
        me.id as _
    )
    .execute(&mut *transaction)
...
async fn register_racetime_inner(
    pool: &State<PgPool>,
    me: Option<User>,
    racetime_user: Option<RaceTimeUser>,
    redirect_uri: Option<rocket::http::uri::Origin<'static>>,
) -> Result<Redirect, RegisterError> {
    Ok(if let Some(racetime_user) = racetime_user {
        let mut transaction = pool.begin().await?;
        if sqlx::query_scalar!(
            r#"SELECT EXISTS (SELECT 1 FROM users WHERE racetime_id = $1) AS "exists!""#,
            racetime_user.id
        )
...
async fn register_discord_inner(
    pool: &State<PgPool>,
    me: Option<User>,
    ...
    Ok(if let Some(discord_user) = discord_user {
        let mut transaction = pool.begin().await?;
        if sqlx::query_scalar!(
            r#"SELECT EXISTS (SELECT 1 FROM users WHERE discord_id = $1) AS "exists!""#,
            PgSnowflake(discord_user.id) as _
        )
        .fetch_one(&mut *transaction)
        .await?
        {
...
pub(crate) async fn register_racetime(
    pool: &State<PgPool>,
    me: Option<User>,
    racetime_user: Option<RaceTimeUser>,
) -> Result<Redirect, RegisterError> {
...
pub(crate) async fn register_discord(
    pool: &State<PgPool>,
    me: Option<User>,
    discord_user: Option<DiscordUser>,
) -> Result<Redirect, RegisterError> {
...
pub(crate) async fn merge_accounts(
    ...
    racetime_user: Option<RaceTimeUser>,
    discord_user: Option<DiscordUser>,
) -> Result<Redirect, MergeAccountsError> {
    let mut transaction = pool.begin().await?;
...
pub(crate) async fn unlink_racetime(
    pool: &State<PgPool>,
    me: User,
    csrf: Option<CsrfToken>,
    cookies: &CookieJar<'_>,
    form: Form<Contextual<'_, UnlinkForm>>,
) -> Result<Redirect, UnlinkError> {
    if me.discord.is_none() {
...
pub(crate) async fn unlink_discord(
    pool: &State<PgPool>,
    me: User,
    csrf: Option<CsrfToken>,
    cookies: &CookieJar<'_>,
...
pub(crate) async fn unlink_startgg(
    pool: &State<PgPool>,
    me: User,
    csrf: Option<CsrfToken>,
    form: Form<Contextual<'_, UnlinkForm>>,
    ...
    if form.value.is_some() {
        sqlx::query!(
            "UPDATE users SET startgg_id = NULL WHERE id = $1",
            me.id as _
        )
        .execute(&**pool)
        .await?;
...
pub(crate) async fn unlink_challonge(
    pool: &State<PgPool>,
    me: User,
    csrf: Option<CsrfToken>,
    form: Form<Contextual<'_, UnlinkForm>>,
    ...
    if form.value.is_some() {
        sqlx::query!(
            "UPDATE users SET challonge_id = NULL WHERE id = $1",
            me.id as _
        )
        .execute(&**pool)
        .await?;
