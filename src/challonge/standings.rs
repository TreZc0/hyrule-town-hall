...
use super::import::{fetch_matches, fetch_participants};
use crate::prelude::*;
...
pub(crate) async fn swiss_standings(
    ...
    let matches =
        fetch_matches(http_client, config, community, tournament, Some("complete")).await?;

    // Build participant ID -> name map and win/loss records
    let mut records: HashMap<&str, (usize, usize)> = participants
        .iter()
        .map(|p| (p.id.as_str(), (0, 0)))
        .collect();

    for m in &matches {
        let Some(winner_id) = m.attributes.as_ref().and_then(|a| a.winner_id) else {
            continue;
        };
        ...
        match (p1, p2) {
            (Some(p1), Some(p2)) => {
                ...
                if let Some(r) = records.get_mut(winner_id_str.as_str()) {
                    r.0 += 1;
                }
                if let Some(r) = records.get_mut(loser_id) {
                    r.1 += 1;
                }
            }
            _ => {
    ...
    let mut standings: Vec<startgg::SwissStanding> = participants
        .iter()
        .map(|p| {
