...
fn check_static_file(
    ...
    let commit_id = loop {
        ...
        let diff = repo.diff_tree_to_tree(
            Some(&parent.tree()?),
            Some(&iter_commit.tree()?),
            Some(git2::DiffOptions::default().pathspec(&path)),
        )?;
...
fn main() -> Result<(), Error> {
    ...
    let mut out_f =
        File::create(Path::new(&env::var_os("OUT_DIR").unwrap()).join("static_files.rs"))?;
    ...
    for (path, commit_id) in cache {
        let unix_path = path
            .to_str()
            .expect("non-UTF-8 static file path")
            .replace('\\', "/");
    ...
    for (unix_path, commit_id) in &normal_entries {
        ...
        writeln!(
            &mut out_f,
            "        ::rocket_util::Origin(::rocket::uri!({uri:?}))"
        )?;
        writeln!(&mut out_f, "    }};")?;
    }
    writeln!(&mut out_f, "}}")?;
